...
- 2026-04-25: closed `V1DEPLOY-2026-04-25-B` as the final deployed-commit proof hardening slice for `soar-web`. `scripts/writeWebBuildMetadata.mjs` now resolves commit truth from minimal `.git` metadata files when build env hints are absent, `.dockerignore` preserves only the minimal `.git` files needed for that resolution, and `apps/web/Dockerfile` copies `.git` into the build stage so Coolify Docker builds can emit deploy-verifiable `gitSha` without depending on unavailable provider envs. Validation PASS: `pnpm --filter web run build`, `docker build -f apps/web/Dockerfile -t soar-web-gitsha-fix .`.
