...
- 2026-04-30: closed `V1SAFE-12` as the immediate follow-up to the trailing-threshold safety hardening. After `V1SAFE-11`, production strategy edit on the real `LIVE` strategy still failed even with the bot disabled because the stored config contained one legacy invalid `TSL` level (`arm=10`, `percent=-20`), and the new fail-closed validator correctly blocked every submit, including unrelated `TTP` changes. The repository now sanitizes legacy invalid advanced `TTP/TSL` thresholds when loading strategy DTOs into the web form, preserving only levels that satisfy the same protective-close contract enforced by API/runtime validation. This keeps new writes fail-closed without trapping operators behind old invalid config they can no longer save through the UI.
