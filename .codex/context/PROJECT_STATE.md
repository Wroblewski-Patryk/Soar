...
- Current phase: V1 is approved from the current repository evidence set, with
  ...
  user-approved scope is therefore `V1LIVE-A`, an exchange-selected live
  hardening wave that keeps `PAPER` exchange-free, keeps all `LIVE` work inside
  the approved exchange boundary, forces adapter selection to follow the exact
  user/bot `exchange + marketType` settings, and closes the full path
  `signal -> order -> exchange update -> position -> takeover/runtime
  visibility` under one canonical contract. The first implemented adapter
  family in that wave is `BINANCE + SPOT` and `BINANCE + FUTURES`; it is not a
  hidden default for all future live execution. The newest post-V1 production hotfix slice
...
- 2026-04-26: active `V1LIVE-PROD-2026-04-26-A` closure work proved that the last real-account manual-order blocker is web-only stale symbol-context drift. Browser reproduction on production showed the dashboard could submit `symbol=DOGEUSDT` with a stale previous-symbol price from `1000000BOBUSDT`, even though `/dashboard/orders/manual-context` and direct authenticated `/dashboard/orders/open` were both truthful. `useManualOrderController.ts` now accepts manual-order context price only when it matches the current `selected.bot.id + manualOrderSymbol`, preventing previous-symbol context from freezing the wrong price into submit state. Focused validation PASS: `pnpm --filter web exec vitest run src/features/dashboard-home/components/HomeLiveWidgets.manual-order.test.tsx src/features/dashboard-home/hooks/useManualOrderController.test.tsx`, `pnpm --filter web run typecheck`, `pnpm run quality:guardrails`, `pnpm --filter web run build`. Remaining step is operational: redeploy `soar-web` and rerun the exact browser flow on the real account.
- 2026-04-26: active `V1LIVE-PROD-2026-04-26-B` follow-up work on the same real production account proved there is still one live-position truth gap after the earlier takeover/manual-order closure. Current production evidence shows the exchange snapshot surface can still degrade imported leverage to `null` even though Binance Futures returns it inside nested raw payload fields, and stale local managed LIVE positions can survive in runtime/dashboard when the main position snapshot succeeds but the auxiliary open-orders snapshot fails in the same reconciliation pass. The repository now normalizes leverage and related numeric truth from nested raw exchange payload fields, and `livePositionReconciliation` treats open-orders fetch as fail-soft for stale local cleanup, using a stricter fallback grace window instead of preserving old ghost rows forever. Validation PASS: `pnpm --filter api exec vitest run src/modules/positions/livePositionReconciliation.service.test.ts`, `pnpm --filter api run typecheck`, `pnpm run quality:guardrails`, `pnpm --filter api run build`. Remaining work is operational: deploy the API fix to production, run user-scoped repair on the affected account, and verify that stale `BNBUSDT` disappears while imported `DOGEUSDT` stops falling back to `1x` leverage semantics.
