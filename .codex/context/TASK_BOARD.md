...
- [ ] `V1LIVE-PROD-2026-04-26-B api(prod-position-truth): recover imported leverage truth and stale local LIVE cleanup on the affected account`
  - 2026-04-26: Fresh production audit on the real live bot proved two residual truth gaps after the earlier browser and lifecycle fixes: imported Binance Futures positions can still surface `leverage=null` in the exchange snapshot path even though runtime margin/PnL% depend on real leverage, and one historical local `BOT` row (`BNBUSDT`) remains visible in runtime/dashboard even when the current exchange snapshot confirms only `DOGEUSDT`. The repository fix now reads leverage and related fields from nested raw exchange payload fields during snapshot normalization, and `livePositionReconciliation` no longer lets a failing open-orders snapshot block stale local managed LIVE cleanup for sufficiently old rows. Validation PASS locally: `pnpm --filter api exec vitest run src/modules/positions/livePositionReconciliation.service.test.ts`, `pnpm --filter api run typecheck`, `pnpm run quality:guardrails`, `pnpm --filter api run build`. Remaining step: deploy API to prod, run repair on the affected account, and verify that stale `BNBUSDT` disappears while imported `DOGEUSDT` no longer degrades to `1x`.
