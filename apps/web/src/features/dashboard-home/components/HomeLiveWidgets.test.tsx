...
describe("HomeLiveWidgets", () => {
  ...
  it("renders source column in open orders table with origin mapping", async () => {
    listBotsMock.mockResolvedValue([
      {
    ...
    listBotRuntimeSessionSymbolStatsMock.mockResolvedValue({
      sessionId: "session-orders-source",
      items: [],
      summary: {
        totalSignals: 0,
        longEntries: 0,
    ...
    listBotRuntimeSessionPositionsMock.mockResolvedValue({
      ...
      openOrders: [
        ...
        },
        {
          ...
          price: 3000,
          stopPrice: null,
          submittedAt: "2026-03-31T10:03:00.000Z",
    ...
    const ordersTab = await screen.findByRole("tab", { name: /Orders/i });
    fireEvent.click(ordersTab);

    const table = await screen.findByRole("table");
    expect(within(table).getByRole("columnheader", { name: /Source/i })).toBeInTheDocument();

    const manualRow = within(table).getByText("BTCUSDT").closest("tr");
    const botRow = within(table).getByText("ETHUSDT").closest("tr");
    const importedRow = within(table).getByText("ADAUSDT").closest("tr");

    expect(manualRow).not.toBeNull();
    expect(botRow).not.toBeNull();
    expect(importedRow).not.toBeNull();

    expect(within(manualRow as HTMLElement).getByText("Manual")).toBeInTheDocument();
    expect(within(botRow as HTMLElement).getByText("Bot")).toBeInTheDocument();
    expect(within(importedRow as HTMLElement).getByText("Imported")).toBeInTheDocument();
  });
