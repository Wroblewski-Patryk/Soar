...
describe('reconcileExternalPositionsFromExchange', () => {
  ...
  it('uses api-key exchange when hydrating owned LIVE automation after exchange-sync create', async () => {
    const processOwnedSyncedPositionAutomation = vi.fn(async () => undefined);

    await reconcileExternalPositionsFromExchange({
      listSyncedApiKeys: vi.fn(async () => [
        {
          id: 'key-live-automation-gateio-1',
          userId: 'user-live-automation-gateio-1',
      ...
      resolveOwnershipIndexForUser: vi.fn(async () =>
        new Map([
          [
            'key-live-automation-gateio-1:ADAUSDT',
            {
              status: 'OWNED' as const,
              botId: 'bot-live-automation-gateio-1',
              walletId: 'wallet-live-automation-gateio-1',
            },
          ],
        ])
      ),
      ...
      resolveCanonicalBotContinuityContext: vi.fn(async () => ({
        botId: 'bot-live-automation-gateio-1',
        walletId: 'wallet-live-automation-gateio-1',
        strategyId: 'strategy-live-automation-gateio-1',
      })),
      updateSyncedPosition: vi.fn(async () => undefined),
      createSyncedPosition: vi.fn(async () => undefined),
      listOpenSyncedPositionsForApiKey: vi.fn(async () => []),
  ...
  it('uses api-key exchange when hydrating owned LIVE automation after exchange-sync update', async () => {
    const processOwnedSyncedPositionAutomation = vi.fn(async () => undefined);

    await reconcileExternalPositionsFromExchange({
      listSyncedApiKeys: vi.fn(async () => [
        {
          id: 'key-live-automation-gateio-update-1',
          userId: 'user-live-automation-gateio-update-1',
          exchange: 'GATEIO' as const,
          marketType: 'FUTURES' as const,
        },
      ]),
      resolveOwnershipIndexForUser: vi.fn(async () =>
        new Map([
          [
            'key-live-automation-gateio-update-1:ADAUSDT',
            {
              status: 'OWNED' as const,
              botId: 'bot-live-automation-gateio-update-1',
              walletId: 'wallet-live-automation-gateio-update-1',
            },
          ],
        ])
      ),
      ...
      findOpenSyncedPositionByExternalId: vi.fn(async () => ({
        id: 'pos-live-automation-gateio-update-1',
        botId: 'bot-live-automation-gateio-update-1',
        walletId: 'wallet-live-automation-gateio-update-1',
        strategyId: 'strategy-live-automation-gateio-update-1',
        openedAt: new Date('2026-03-23T00:05:00.000Z'),
        managementMode: 'BOT_MANAGED' as const,
        continuityState: 'CONFIRMED' as const,
        missingSyncCount: 0,
