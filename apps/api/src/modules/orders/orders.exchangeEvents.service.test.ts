...
describe('orders.exchangeEvents.service', () => {
  ...
  it('keeps fee pending when a final fill arrives without fee after a partial exact fee', async () => {
    const user = await prisma.user.create({
      data: { email: 'exchange-event-final-without-fee-after-partial@example.com', password: 'hashed' },
    });
    const wallet = await prisma.wallet.create({
      data: {
        userId: user.id,
        name: 'live-wallet-final-without-fee-after-partial',
        mode: 'LIVE',
        exchange: 'BINANCE',
        marketType: 'FUTURES',
        baseCurrency: 'USDT',
    ...
    const bot = await prisma.bot.create({
      data: {
        userId: user.id,
        name: 'live-bot-final-without-fee-after-partial',
        mode: 'LIVE',
        exchange: 'BINANCE',
        marketType: 'FUTURES',
        positionMode: 'ONE_WAY',
    ...
    const order = await prisma.order.create({
      data: {
        ...
        feeSource: 'ESTIMATED',
        feePending: false,
        exchangeOrderId: 'event-final-without-fee-after-partial-1',
        submittedAt: new Date('2026-04-26T20:08:40.000Z'),
      },
    });
    ...
    await applyLiveExchangeOrderTradeUpdateEvent({
      ...
      event: {
        ...
        executionType: 'TRADE',
        exchangeOrderId: 'event-final-without-fee-after-partial-1',
        clientOrderId: 'client-final-without-fee-after-partial-1',
        averagePrice: 3_000,
        cumulativeFilledQuantity: 0.5,
        lastFilledQuantity: 0.5,
        lastFilledPrice: 3_000,
        fee: 0.01,
        feeCurrency: 'USDT',
        exchangeTradeId: 'trade-final-without-fee-after-partial-1',
        raw: {},
      },
    });
    ...
    await applyLiveExchangeOrderTradeUpdateEvent({
      ...
      event: {
        ...
        fee: null,
        feeCurrency: null,
        exchangeTradeId: 'trade-final-without-fee-after-partial-2',
        raw: {},
      },
    });
    ...
    expect(updatedOrder.status).toBe('FILLED');
    expect(updatedOrder.filledQuantity).toBeCloseTo(1, 10);
    expect(updatedOrder.fee).toBeCloseTo(0.01, 10);
    expect(updatedOrder.feeSource).toBe('EXCHANGE_FILL');
    expect(updatedOrder.feePending).toBe(true);
    const fills = await prisma.orderFill.findMany({
    ...
    expect(trade.fee).toBeCloseTo(0.01, 10);
    expect(trade.feeSource).toBe('EXCHANGE_FILL');
    expect(trade.feePending).toBe(true);
  });

  it('keeps fee pending when a final exact fee leaves an earlier partial fill without fee', async () => {
    const user = await prisma.user.create({
      data: { email: 'exchange-event-final-fee-after-missing-partial@example.com', password: 'hashed' },
    });
    const wallet = await prisma.wallet.create({
      data: {
        userId: user.id,
        name: 'live-wallet-final-fee-after-missing-partial',
        mode: 'LIVE',
        exchange: 'BINANCE',
        marketType: 'FUTURES',
        baseCurrency: 'USDT',
    ...
    const bot = await prisma.bot.create({
      data: {
        userId: user.id,
        name: 'live-bot-final-fee-after-missing-partial',
        mode: 'LIVE',
        exchange: 'BINANCE',
        marketType: 'FUTURES',
        positionMode: 'ONE_WAY',
    ...
    const order = await prisma.order.create({
      data: {
        ...
        feeSource: 'ESTIMATED',
        feePending: false,
        exchangeOrderId: 'event-final-fee-after-missing-partial-1',
        submittedAt: new Date('2026-04-26T20:08:50.000Z'),
      },
    });
    ...
    await applyLiveExchangeOrderTradeUpdateEvent({
      ...
      event: {
        ...
        executionType: 'TRADE',
        exchangeOrderId: 'event-final-fee-after-missing-partial-1',
        clientOrderId: 'client-final-fee-after-missing-partial-1',
        averagePrice: 3_000,
        cumulativeFilledQuantity: 0.5,
        lastFilledQuantity: 0.5,
        lastFilledPrice: 3_000,
        fee: null,
        feeCurrency: null,
        exchangeTradeId: 'trade-final-fee-after-missing-partial-1',
        raw: {},
      },
    });
    ...
    await applyLiveExchangeOrderTradeUpdateEvent({
      ...
      event: {
        ...
        fee: 0.02,
        feeCurrency: 'USDT',
        exchangeTradeId: 'trade-final-fee-after-missing-partial-2',
        raw: {},
      },
    });
    ...
    expect(updatedOrder.status).toBe('FILLED');
    expect(updatedOrder.filledQuantity).toBeCloseTo(1, 10);
    expect(updatedOrder.fee).toBeCloseTo(0.02, 10);
    expect(updatedOrder.feeSource).toBe('EXCHANGE_FILL');
    expect(updatedOrder.feePending).toBe(true);
    const fills = await prisma.orderFill.findMany({
      where: { orderId: order.id },
    ...
    expect(trade.fee).toBeCloseTo(0.02, 10);
    expect(trade.feeSource).toBe('EXCHANGE_FILL');
    expect(trade.feePending).toBe(true);
  });
