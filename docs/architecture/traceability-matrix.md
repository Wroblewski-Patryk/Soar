...
| Profile and API keys | `/dashboard/profile`, `features/profile` | `/dashboard/profile/basic`, `/dashboard/profile/security`, `/dashboard/profile/apiKeys`, `/upload/avatar` | `api/profile`, `api/upload`, `api/exchange` API-key probe | `User`, `ApiKey` | Account/API-key setup | `profile/*/*.e2e.test.ts`, `binanceApiKeyProbe.service.test.ts`, `upload.e2e.test.ts` | `docs/modules/api-profile.md`, `docs/security/api-key-lifecycle-policy.md` |
