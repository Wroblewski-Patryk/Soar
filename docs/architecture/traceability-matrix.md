...
| Bot runtime monitoring | `/dashboard`, `/dashboard/bots*`, `features/dashboard-home`, `features/bots` | `/dashboard`, `/dashboard/bots/:id/runtime-*`, `/dashboard/market-stream/events`, `/dashboard/icons/lookup` | `api/bots` runtime read models, `api/market-stream`, `api/icons`, workers | `BotRuntimeSession`, `BotRuntimeEvent`, `BotRuntimeSymbolStat`, `Position`, `Trade`, `Signal` | Runtime signal execution | runtime read-model tests, market stream tests, dashboard/bots component tests | `docs/modules/api-bots.md`, `docs/architecture/reference/stream-transport-contract.md` |
...
- this matrix,
- `docs/architecture/architecture-evidence-graph-system.md` and the CSV graph
  registries when the change adds or changes a node, relation, chain, test,
  or documentation proof,
