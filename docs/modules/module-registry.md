...
| exchanges | Exchange connection context UI/redirect. | `/dashboard/exchanges` | Account/API-key setup | profile API keys | exchange feature tests if present | `web-exchanges.md` | Standalone write surface is intentionally not current canonical path. |
