...
Last classified: 2026-07-12 under [LUC-636](/LUC/issues/LUC-636).
...
| `apps/api/src/modules/auth/auth.session.ts#getSessionJwtExpiresIn`         | `docs/modules/api-auth.md` | Account access session-token TTL selector that returns the short-lived JWT expiry for standard sessions and the extended remember-me expiry for persistent sessions.                                                            | Architecture-awareness `documents` relation from this doc plus auth service/controller session TTL proof when remember-me JWT expiry behavior changes.                                                                             |
| `apps/api/src/modules/auth/auth.session.ts#getSessionTtlMs`                | `docs/modules/api-auth.md` | Account access cookie/session TTL selector that returns the short-lived millisecond duration for standard sessions and the extended remember-me duration for persistent sessions.                                               | Architecture-awareness `documents` relation from this doc plus auth controller cookie TTL proof when remember-me cookie lifetime behavior changes.                                                                                 |
| `apps/api/src/modules/auth/sessionToken.test.ts#makeRequest`               | `docs/modules/api-auth.md` | Session-token proof helper that builds minimal Express request objects so candidate extraction can verify bearer tokens, parsed cookies, and duplicate raw cookie headers without a live HTTP server.                           | Architecture-awareness `documents` relation from this doc plus focused `sessionToken.test.ts` proof when session candidate extraction behavior changes.                                                                            |
| `apps/api/src/modules/auth/sessionToken.test.ts#restoreEnv`                | `docs/modules/api-auth.md` | Session-token proof helper that restores JWT secret environment variables after each candidate-verification scenario so rotation and signing configuration does not leak between cases.                                         | Architecture-awareness `documents` relation from this doc plus focused `sessionToken.test.ts` proof when JWT environment isolation for session-token tests changes.                                                                |
| `apps/api/src/modules/auth/sessionToken.test.ts#signCandidate`             | `docs/modules/api-auth.md` | Session-token proof helper that signs controlled JWT candidates with the auth issuer and audience so candidate validation can cover invalid claims, duplicate cookie precedence, and issued-at ordering.                        | Architecture-awareness `documents` relation from this doc plus focused `sessionToken.test.ts` proof when session candidate signing assumptions or claim-shape coverage changes.                                                    |
| `apps/api/src/modules/auth/sessionToken.ts#tokenIssuedAt`                  | `docs/modules/api-auth.md` | Session-token ordering helper that normalizes missing or invalid JWT `iat` claims to zero before sorting verified session candidates by newest issued-at value and original candidate order.                                    | Architecture-awareness `documents` relation from this doc plus focused `sessionToken.test.ts` proof when duplicate-token ordering or candidate precedence changes.                                                                 |
| `apps/api/src/modules/auth/auth.service.ts#registerUser`                   | `docs/modules/api-auth.md` | Registration service path that rejects duplicate email addresses, hashes the password, creates the public account with the default avatar, and bootstraps the default subscription inside the same database transaction.        | Architecture-awareness `documents` relation from this doc plus DB-backed `auth.service.test.ts` and register route proof when account creation, duplicate rejection, password hashing, or subscription bootstrap behavior changes. |
| `apps/api/src/modules/auth/auth.service.ts#loginUser`                      | `docs/modules/api-auth.md` | Login service path that loads the account by email, validates the stored password hash, signs a remember-aware session token with `sessionVersion`, and returns only public user fields with the token.                         | Architecture-awareness `documents` relation from this doc plus service/controller login proof when credential validation, token payload, or public user response behavior changes.                                                 |
| `apps/api/src/modules/auth/auth.jwt.ts#signAuthToken`                      | `docs/modules/api-auth.md` | JWT signing helper that builds account-session tokens using the configured remember-aware expiry and primary signing secret, failing closed if signing configuration is invalid or missing.                                     | Architecture-awareness `documents` relation from this doc plus `auth.jwt.test.ts` and `auth.controller` token bootstrap flows when JWT token signing behavior changes.                                                             |
