...
## Phase V1LIVE-A - Binance Futures Live Execution and Takeover Hardening (Queued 2026-04-26)
...
### Progress Log (Phase V1LIVE-A - Binance Futures Live Execution and Takeover Hardening)
- 2026-04-26: Queued `V1LIVE-A` after a fresh user-driven regression analysis on live positions and signal-driven bot opens. The repository-level conclusion is that the remaining issue is architectural drift, not one isolated manual-order bug: ownership truth still splits between reconciliation and runtime, imported live entry truth still degrades to `markPrice`, runtime visibility/close parity for owned `EXCHANGE_SYNC BOT_MANAGED` positions is not stable, and Binance Futures lifecycle truth still relies mainly on REST snapshots instead of the approved exchange boundary plus user-data-stream events. Published the canonical packet `docs/planning/v1live-binance-execution-and-takeover-hardening-plan-2026-04-26.md` and the matching planning task `docs/planning/v1live-00-planning-task-2026-04-26.md`. The wave intentionally keeps `PAPER` exchange-free, keeps all `LIVE` work inside `modules/exchange`, and schedules red tests before fixes so signal -> order -> fill -> position -> takeover can be closed once under one contract.
