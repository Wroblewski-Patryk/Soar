...
- [ ] `V1ROE-04 qa(prod-manual): verify exchange-aligned LIVE PnL truth and DCA non-trigger on the protected DOGEUSDT flow`
  - 2026-04-30 status: READY. After the isolated-margin normalization fix deploys, rerun the protected production/operator check on the affected `DOGEUSDT` lifecycle. Success means Soar `PnL %` is aligned with the exchange closely enough for operator trust and `DCA` stays idle when the exchange-aligned drawdown remains above the configured threshold boundary such as about `-19%` versus `DCA=-25%`.
