...
- [ ] `V1ROE-03 qa(prod-manual): verify exchange-aligned LIVE PnL truth and DCA non-trigger on the protected DOGEUSDT flow`
  - 2026-04-30 status: READY. The code path now uses canonical `marginUsed` truth by mode, but the exact reported `DOGEUSDT` production lifecycle still needs authenticated protected verification after deploy. The latest protected prod check still returns the old payload shape without `marginUsed` / `unrealizedPnlPercent`, so success here now explicitly means both deploy freshness and operator parity: Soar `PnL %` is aligned with the exchange closely enough for operator trust and `DCA` stays idle when the exchange-aligned drawdown remains above the configured threshold boundary such as about `-19%` versus `DCA=-25%`.
