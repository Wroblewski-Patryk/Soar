...
- [ ] `V1ROE-04 qa(prod-manual): verify exchange-aligned LIVE PnL truth and DCA non-trigger on the protected DOGEUSDT flow`
  - 2026-04-30 status: READY. The protected production audit after `V1ROE-03` first exposed stale runtime price truth, and the following ownership-parity audits exposed two deeper seams: imported owned `EXCHANGE_SYNC` rows could still be skipped by runtime automation/bot-scope counting when the canonical row kept `botId=null`, and freshly adopted imported rows could still stay dormant until a later ticker-path event arrived. Both local gaps are now fixed and covered. Next proof after deploy: the active `DOGEUSDT` runtime session must align with exchange truth and imported management must no longer look dormant.
