...
- [ ] `V1ROE-04 qa(prod-manual): verify exchange-aligned LIVE PnL truth and DCA non-trigger on the protected DOGEUSDT flow`
  - 2026-04-30 status: READY. The protected production audit after `V1ROE-03` proved the last local code gap was narrower than isolated-margin normalization alone: exchange sync persisted fresh `Position` truth, but runtime session `positions` and `symbol-stats` could still recompute operator truth from an older runtime stat/ticker price. That read-model freshness seam is now fixed and locked by focused API e2e. Next proof after deploy: `DOGEUSDT` runtime session views must align closely with exchange truth and keep `DCA` idle when drawdown remains above `-25%`.
