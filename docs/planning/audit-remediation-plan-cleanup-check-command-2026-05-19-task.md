...
## Header
- ID: AUDIT-REMEDIATION-PLAN-CLEANUP-CHECK-COMMAND-2026-05-19
- Title: Require cleanup checks in remediation plan
- Task Type: feature
- Current Stage: verification
- Status: DONE
- Owner: Ops/Release
- Depends on: `AUDIT-TOOLING-INDEX-SELF-CHECK-CLOSURE-2026-05-19`
- Priority: P1
...
- Mission ID: `AUDIT-REMEDIATION-CLEANUP-HARDENING`
...
- [x] The task improves remediation-plan cleanup repeatability without changing runtime behavior.

## Mission Block
- Mission objective: make the remediation plan validator fail if required cleanup checks are missing.
- Release objective advanced: future remediation roadmaps keep local validation cleanup explicit.
...
- Handoff expectation: `audit:remediation-plan:check` fails when cleanup checks omit headless browser, local DB/Redis listener, or Docker compose checks.

## Context

The remediation master plan listed concrete cleanup checks, but its validator only verified closure checks, blockers, references, phases, work packages, and safety boundaries. The other reusable audit validators now require exact cleanup checks, so remediation plan validation should enforce the same cleanup contract.

## Goal

Harden `audit:remediation-plan:check` so cleanup checks must include the expected headless browser, local DB/Redis listener, and Docker compose checks.
...
## Success Signal
- User or operator problem: future remediation plans could omit important cleanup checks and still pass validation.
- Expected reliability outcome: remediation plan validation fails if required cleanup check fragments are absent.
- How success will be observed: remediation plan check reports zero missing cleanup checks.
- Post-launch learning needed: no.

## Deliverable For This Stage

A stricter remediation-plan validator with regression coverage for missing required cleanup checks.
...
## Implementation Plan
1. Add required cleanup-check fragments to `checkAuditRemediationPlan.mjs`.
...
## Acceptance Criteria
- [x] Remediation plan validator requires `chrome-headless-shell` cleanup visibility.
- [x] Remediation plan validator requires `Get-NetTCPConnection` local listener visibility.
- [x] Remediation plan validator requires `docker compose ps`.
...
- Tests:
  ...
  - `git diff --check`
- Manual checks: reviewed remediation plan JSON cleanup checks.
...
- Architecture source reviewed: project memory index, audit remediation master plan, reusable audit tooling index
...
- Issues: remediation plan cleanup validation only checked that cleanup data existed in the plan, not that required cleanup checks were enforced.
- Gaps: the plan could lose headless browser, local listener, or Docker cleanup checks without failing `audit:remediation-plan:check`.
- Inconsistencies: tooling-index and rerun-playbook cleanup validation were stricter than remediation-plan cleanup validation.
- Architecture constraints: keep this local and evidence-scoped.

### 2. Select One Priority Mission Objective
- Selected task: remediation-plan cleanup check hardening.
- Priority rationale: keeps remediation roadmap closure self-cleaning while protected `AUD-19` execution remains blocked.
- Why other candidates were deferred: protected production evidence requires missing approved inputs.

### 3. Plan Implementation
- Files or surfaces to modify: remediation plan validator/test, tooling index docs, and state files.
...
- Changed validator to compute required cleanup check gaps.
...
- Task summary: remediation-plan validation now catches missing required cleanup checks.
- Files changed: remediation plan validator/test, tooling index Markdown/JSON, this task record, and source-of-truth state docs.
- How tested: focused remediation-plan tests/check, full audit manifest verification, docs parity, guardrails, diff check.
