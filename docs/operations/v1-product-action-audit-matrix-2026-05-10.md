...
| Dashboard Home | selected bot, wallet KPIs, positions/orders/trades tables, TTP/TSL/DCA/PnL rendering | UI table assertions + runtime payload fixtures | `PARTIAL_LOCAL` | Runtime table presenter proof now covers negative/zero/positive protection semantics, DCA/fees/history/trade metadata, blocked position actions, and order cancelability. A rendered `HomeLiveWidgets` proof covers negative-PnL prospective TTP suppression. Broader rendered/browser proof for selected bot, wallet KPIs, loading/empty/error, responsive, table tabs, and production-safe clickthrough remains open. |
