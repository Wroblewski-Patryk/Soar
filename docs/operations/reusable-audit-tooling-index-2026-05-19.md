...
| `AUDIT-ROLLUP-CHECK` | `corepack pnpm run audit:rollup:check` | `scripts/checkFullReusableAuditRollup.mjs` | Validate full reusable audit rollup coverage, Markdown/JSON audit ID coverage, summary counts, source paths, repair queue, and safety booleans. |
| `AUDIT-ROLLUP-CHECK-TEST` | `corepack pnpm run audit:rollup:check:test` | `scripts/checkFullReusableAuditRollup.test.mjs` | Regression-test full reusable audit rollup validation behavior, including Markdown/JSON audit ID drift. |
