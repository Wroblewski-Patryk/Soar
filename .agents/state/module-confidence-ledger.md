...
- 2026-05-23 post-release docs/state deploy freshness: follow-up docs/state
  ...
  `878e199dd13cabc9a8a25b1ece83d0c483ec0c22`; it required cancelling stale
  queued/in-progress worker/API Coolify deployments, fixing Web Docker
  build-arg scope for deploy metadata, triggering a fresh `soar-web` deploy,
  waiting for public Web build-info to converge to `878e199d`, and passing
  public smoke for API `/health`, API `/ready`, and Web `/`. Authenticated
