...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| SOAR-AUTH-001 | Auth | Login, logout, session validation, expired-session redirect, and protected-route cookie gate | P0 | PARTIAL | High | 2026-05-11 `V1-AUTH-SESSION-LIFECYCLE-PROOF-2026-05-11`: API Auth e2e passed (`11/11`) and proves registration/login cookie TTLs, logout cookie clearing with subsequent `/auth/me` fail-closed behavior, deleted-user session expiry, expired JWT cookie clearing with session-expired message, and duplicate token precedence. Focused Web Auth tests passed (`5` files, `17` tests) and cover AuthProvider bootstrap, logout redirect, session-expired warning cleanup, API interceptor redirect to `/auth/login?session=expired`, middleware cookie gate, login form rendering/error alert, and login hook fail-closed missing-session-refresh behavior. 2026-05-14 `docs/operations/prod-auth-session-browser-proof-2fc90a08-2026-05-14.md` found a production replay gap on deployed `2fc90a08`: browser route fail-closed checks passed, but direct reuse of the pre-logout JWT still returned `/auth/me` `200`. Local fix `V1-POST-V1-AUTH-LOGOUT-TOKEN-REUSE-HARDENING-2026-05-14` reuses existing `sessionVersion` invalidation on logout; focused Auth/middleware tests passed (`21/21`) and API typecheck passed. | Production deployed build `2fc90a08` still has the stale-token reuse gap until the fix is deployed and the production auth browser proof is rerun. | Deploy the auth logout invalidation fix, then rerun `pnpm run ops:prod-auth:proof` against the new build and promote only if it passes. | Backend Builder + QA/Test | 2026-05-14 |
