...
| SOAR-OPERATIONS-001 | Operations | Deployment smoke, rollback guard/proof, SLO evidence, release gates, alerts, backup/restore, and liveimport readback | P0 | PARTIAL | High | 2026-05-13 `V1-TARGET-RELEASE-GATE-PASS-00169D7F-2026-05-13`: final `LIVEIMPORT-03` passed for `TRXUSDT`; final preflight has no blockers; production target-only V1 release gate is `ready`; build-info freshness, post-deploy smoke, runtime freshness, and rollback guard passed against deployed `00169d7f`. 2026-05-14 `V1-CURRENT-MAIN-PROMOTION-DEPLOY-LAG-457BCE05-2026-05-14`: current candidate `457bce05` is pushed to `origin/main`, but production build-info remained on `00169d7f` during the bounded wait; public production smoke still passes for the currently deployed surface. | The latest code candidate is not yet deployed, so Operations cannot be `VERIFIED` for current `main`. Full Docker-backed local go-live smoke was not rerun because Docker Desktop is unavailable on this workstation. Separate LIVE+PAPER simultaneous runtime proof remains a follow-up outside the deploy-lag check. | Retrigger or inspect production Coolify deployment for `457bce05`, then rerun build-info freshness, production smoke, and the target release gate; rerun Docker-backed local smoke when Docker Desktop is available. | Ops/Release + QA/Test | 2026-05-14 |
...
- 2026-05-14
  ...
  surface. The row is `PARTIAL` for current `main` until Coolify deploys
  `457bce05` and the target release gate is rerun for that SHA.
- 2026-05-13
