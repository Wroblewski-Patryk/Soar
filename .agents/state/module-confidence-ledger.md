...
| SOAR-OPERATIONS-001 | Operations | Deployment smoke, rollback guard/proof, SLO evidence, release gates, alerts, backup/restore, and liveimport readback | P0 | PARTIAL | High | 2026-05-13 `V1-TARGET-RELEASE-GATE-PASS-00169D7F-2026-05-13`: final `LIVEIMPORT-03` passed for `TRXUSDT`; final preflight has no blockers; production target-only V1 release gate is `ready`; build-info freshness, post-deploy smoke, runtime freshness, and rollback guard passed against deployed `00169d7f`. 2026-05-14 `V1-CURRENT-MAIN-PROMOTION-DEPLOY-LAG-457BCE05-2026-05-14`: current candidate `457bce05` is pushed to `origin/main`; later production build-info reported `457bce05`, and public production smoke passed for that deployed surface. Protected runtime freshness and rollback guard checks failed closed with HTTP `401` without approved admin/ops credentials. | The latest code candidate is deployed and public-smoke healthy, but protected ops evidence for current `main` still needs approved admin/ops access. Full Docker-backed local go-live smoke was not rerun because Docker Desktop is unavailable on this workstation. Separate LIVE+PAPER simultaneous runtime proof remains a follow-up outside this deploy check. | Rerun protected runtime freshness, alerts/rollback guard, and the target release gate for `457bce05` with approved admin/ops credentials; rerun Docker-backed local smoke when Docker Desktop is available. | Ops/Release + QA/Test | 2026-05-14 |
