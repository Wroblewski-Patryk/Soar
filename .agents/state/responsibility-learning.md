...
| RLG-005 | 2026-05-24 | `SOAR-FULL-READINESS-COORDINATION-2026-05-23-cleanup` | missing_lane | The mission had runtime, QA, and state context, but no explicit "source-of-truth drift cleanup" lane, so active files kept stale deploy facts after public build-info converged. | 2026-05-24 scan found active mission, project state, task board, next steps, system health, risk/QAS, current focus, known issues, and planning rows still referencing old deploy checkpoints. | Add a source-of-truth drift lane to broad readiness missions before implementation or proof claims; require a search for stale commit/deploy identifiers after public build-info and local `HEAD` are checked. | `.agents/state/active-mission.md`; `.agents/state/current-focus.md`; `.codex/context/PROJECT_STATE.md`; `.codex/context/TASK_BOARD.md` | open |
| RLG-006 | 2026-07-20 | `LUC-1467-REVIEW-PRODUCTIVITY-RESUME-DELTA-2026-07-20` | missing_context | The wake kept rechecking a live blocker, but this checkout still had no sanctioned Paperclip issue-mutation helper or other local live-status path, so the coordinator could only reassert the blocked disposition. | `history/evidence/luc-1467-review-productivity-resume-delta-2026-07-20.md`; current repo truth and task packet. | For future resume-delta wakes that depend on external issue state, verify a live mutation path exists before promising progress; if it does not, record the blocker and stop at evidence/state refresh. | `history/tasks/luc-1467-review-productivity-resume-delta-2026-07-20-task.md`; `.codex/context/TASK_BOARD.md` | open |
