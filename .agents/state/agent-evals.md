...
| AEV-014 | 2026-05-23 | `LIVE-EXCHANGE-EXECUTION-PARITY-2026-05-23` | 5 | 5 | 5 | 4 | 5 | Read-only manual and bot lanes isolated the real Gate.io symbol/rules defect quickly, but the mission cannot reach full proof until deployed code and an explicitly approved minimum live order size are available. | For live-trading parity work, keep mutation proof behind a separate deploy-and-approval checkpoint; use read-only connector probes to compute the minimum executable size before asking for live order approval. | open |
