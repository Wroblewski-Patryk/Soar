...
| --- | --- | --- | --- | --- |
| 2026-05-22 | Public web deploy-proof route hardening | VERIFIED locally: root production probes were reachable externally while local/proxy probes could not reliably reach dynamic public routes. Converted `/auth/login`, `/auth/register`, and `/api/build-info` to static prerendered routes and removed per-request build-info time generation. | Targeted auth cache contract `2/2`; `web build` route output shows `/auth/login`, `/auth/register`, and `/api/build-info` as `Static`; `web typecheck`; `quality:guardrails`; `git diff --check`; local production HTTP smoke `200` for all three routes | Commit/push to `main`, wait for Coolify autodeploy, then collect production readback. |
