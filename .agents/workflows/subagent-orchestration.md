...
2. Identify independent side tasks that can be delegated without overlapping
   write ownership.
3. Confirm that the user, repository policy, or current task explicitly allows
   subagents.
4. Review `.agents/workflows/responsibility-lanes.md`.
5. Draft a subagent brief with objective, owned files or modules, non-goals,
   validation, and expected report.
6. Continue local non-overlapping work while subagents run.
7. Review subagent findings or diffs before accepting them.
8. Integrate accepted work and reject or track unsafe work.
9. Run parent-level validation after integration.
10. Update source-of-truth state once from the coordinator thread.
11. Close the parent task only when every required delegated lane is resolved.
