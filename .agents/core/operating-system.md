...
This file defines the durable operating system for AI agents working on Soar.
It does not replace `AGENTS.md`, `.codex/context/*`, or the architecture docs.
It gives every future "continue", "rob dalej", or "kontynuuj" run a stable
startup path, decision model, reporting model, and project-state update model.

## Startup Protocol

For non-trivial work, the active agent must read this order:
...
1. `docs/architecture/`
2. product and safety contracts
3. `.codex/context/*`
4. `.agents/state/*`
5. planning files
...
When the user says "rob dalej", "rób dalej", "kontynuuj", "continue", "next",
or another short execution nudge, the agent must:
...
- backend: modules, services, DB access, invariants, error handling
...
Regressions are detected by comparing the intended change against:

- architecture contracts in `docs/architecture/`
- module ownership docs in `docs/modules/`
- end-to-end flows in `docs/pipelines/` and `docs/flows/`
- API/client contracts in `docs/contracts/`
- existing tests and validation commands
- `.codex/context/LEARNING_JOURNAL.md`
- current user-reported issues and active queue items

Any detected regression must be recorded in `.agents/state/regression-log.md`
until fixed or explicitly accepted as residual risk.
...
For review work, findings must lead the response. For implementation work,
the summary can be short, but evidence must be present in the task artifact or
state files.
...
Soar is in V1 foundation mode. The agent should optimize for a coherent,
production-grade foundation: stable runtime behavior, backend/frontend/UI
