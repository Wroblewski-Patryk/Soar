# LUC-705 Architecture Repair Backlog Control Map

Last updated: 2026-05-29
Source issue: `LUC-705 [Soar][Architecture Planning] Convert architecture docs into executable repair backlog`
Backlog source: `history/plans/luc-384-architecture-repair-backlog-2026-05-28.md`
Execution baselines: `history/plans/luc-408-architecture-repair-backlog-execution-map-2026-05-28.md`, `history/plans/luc-508-architecture-repair-backlog-control-map-2026-05-28.md`, `history/plans/luc-583-architecture-repair-backlog-control-map-2026-05-29.md`
...
| --- | --- | --- | --- | --- | --- |
| ARB-001 | `LUC-385` + `LUC-616` | done_gated | Product + CTO + AI Runtime | Keep V1 fail-closed scope from `DEC-ARB-001` (no runtime hot-path activation). Reopen only via explicit post-V1 activation lane. | decision_gate |
...
## Remaining Blockers
1. `ARB-006` (`LUC-402`): blocked on protected-input ownership and child execution issuance.

## Decision-Gated (Not Active Blockers)
1. `ARB-001`: gated by accepted `DEC-ARB-001` (post-V1 activation required for any hot-path runtime expansion).
2. `ARB-002`: gated by accepted `DEC-ARB-002` (mobile runtime activation trigger required).

## Closure Rule For This PM Lane
- `LUC-705` is complete when this control map reflects canonical backlog/execution artifacts and project state files are synchronized.
