...
| ARB-006 | Architecture graph reports high proof gaps (function/user-action indices; production/authenticated proof still missing) (`docs/architecture/architecture-evidence-graph-system.md`) | QA + Ops + Security | Verification | critical | Convert high proof gaps into dated protected/public evidence tasks per critical chain. | Fresh evidence artifacts for auth browser, protected runtime readback, and production proof linked to expected SHA. | Protected inputs + Ops window | blocked_on_inputs |
