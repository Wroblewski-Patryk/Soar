...
| --- | --- | --- | --- | --- | --- |
| `AUD-LOG-004` | P1 | open freshness follow-up | Fresh production action-produced audit readback was not rerun. Historical production proof remains accepted for the 2026-05-14 deployment target. | `history/evidence/prod-fixture-action-proof-457bce05-2026-05-14.md`; this audit's local Web/API packs. | Refresh production-safe action-produced readback after future deployments. |
...
| `AUD-LOG-006` | P2 | cross-audit follow-up | `AUD-14` identified explicit wallet create/update/delete audit-log events as a documented wallet follow-up. Logs read surfaces are current, but the wallet command write-pipeline coverage remains outside the logs read module. | `history/audits/wallets-capital-ledger-audit-2026-05-19.md`; `docs/modules/api-wallets.md`. | Decide in a future implementation plan whether wallet command events are required before broader audit-trail completeness claims. |
