...
## Header
- ID: AUDIT-RERUN-PLAYBOOK-SELF-CHECK-CLOSURE-2026-05-19
- Title: Require rerun playbook self-check closure
- Task Type: feature
- Current Stage: verification
- Status: DONE
- Owner: Ops/Release
- Depends on: `AUDIT-HANDOFF-SELF-CHECK-VALIDATION-2026-05-19`
- Priority: P1
...
- Mission ID: `AUDIT-RERUN-PLAYBOOK-SELF-CHECK-CLOSURE`
...
## Mission Block
- Mission objective: prevent reusable audit rerun playbook closure from omitting its own validator.
- Release objective advanced: future audit rerun playbooks remain self-verifying.
- Included slices: rerun playbook validator rule, regression test, playbook closure docs, tooling index docs, source-of-truth state sync.
...
- Handoff expectation: `audit:rerun-playbook:check` fails when rerun playbook `closureChecks` omit `audit:rerun-playbook:check`.

## Context

The reusable audit rerun playbook already exposes `corepack pnpm run audit:rerun-playbook:check`, but its closure set did not require running that self-check directly. A future rerun playbook edit could remove the self-check from closure while still passing validation through broader commands.

## Goal

Harden `audit:rerun-playbook:check` so rerun playbook closure must include the rerun-playbook self-check command.
...
## Success Signal
- User or operator problem: future rerun playbooks could pass validation without explicitly proving the rerun-playbook validator was run.
- Expected reliability outcome: rerun playbook validation fails if `audit:rerun-playbook:check` is absent from closure checks.
- How success will be observed: rerun playbook check reports zero missing required closure checks.
- Post-launch learning needed: no.

## Deliverable For This Stage

A stricter rerun-playbook validator with regression coverage for missing self-check closure.
...
## Implementation Plan
1. Add `audit:rerun-playbook:check` to required rerun-playbook closure fragments.
2. Add the command to the reusable rerun playbook closure set.
3. Update focused regression coverage for incomplete closure checks.
4. Update tooling index purpose text and source-of-truth state.

## Acceptance Criteria
- [x] Rerun playbook validator requires `audit:rerun-playbook:check`.
- [x] Missing rerun-playbook self-check fails focused regression tests.
...
- Tests:
  ...
  - `git diff --check`
- Manual checks: reviewed rerun playbook JSON and Markdown closure checks.
...
- Issues: rerun playbook closure requirements did not include the rerun playbook validator itself.
- Gaps: a future playbook could omit `audit:rerun-playbook:check` from closure and still pass.
- Inconsistencies: remediation and handoff validations were made self-checking while rerun-playbook closure lacked its own direct self-check requirement.
- Architecture constraints: keep this local and evidence-scoped.

### 2. Select One Priority Mission Objective
- Selected task: rerun-playbook self-check closure hardening.
- Priority rationale: keeps reusable audit rerun instructions self-verifying while protected `AUD-19` execution remains blocked.
- Why other candidates were deferred: protected production evidence requires missing approved inputs.

### 3. Plan Implementation
- Files or surfaces to modify: rerun playbook validator/test, rerun playbook docs, tooling index docs, and state files.
- Logic: closure checks must contain `audit:rerun-playbook:check`.
- Edge cases: closure checks with manifest/remediation/docs/diff only must fail.

### 4. Execute Implementation
- Changed validator to require `audit:rerun-playbook:check`.
- Added the self-check command to rerun playbook JSON and Markdown closure sets.
...
- Task summary: rerun-playbook validation now catches closure evidence that omits the rerun-playbook self-check.
- Files changed: rerun playbook validator/test, rerun playbook Markdown/JSON, tooling index Markdown/JSON, this task record, and source-of-truth state docs.
