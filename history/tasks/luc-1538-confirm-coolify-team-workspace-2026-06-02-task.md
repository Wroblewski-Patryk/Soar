...
## Context
`LUC-1538` asks Ops to confirm the expected Coolify team/workspace selector so
...
## Scope
- Paperclip issue context for `LUC-1538`.
...
- Manual checks:
  - `GET /api/issues/LUC-1538/heartbeat-context` -> pass.
...
- Selected task: confirm Coolify team/workspace selector for `LUC-1538`.
- Priority rationale: wrong workspace context can invalidate production evidence.
- Why other candidates were deferred: wake payload scoped this heartbeat to `LUC-1538`.
...
### 6. Self-Review
- Simpler option considered: relying only on previous `LUC-1533` evidence.
- Technical debt introduced: no.
- Scalability assessment: future automation can bind selector id `0` as non-secret guard if desired.
- Refinements made: scoped this checkpoint to selector and project/environment visibility, not full resource reconciliation.

### 7. Update Documentation and Knowledge
- Docs updated: this task packet, evidence packet, operations deployment contract, runtime config ledger.
...
- Files changed: this task packet, evidence packet, operations deployment contract, runtime config ledger.
