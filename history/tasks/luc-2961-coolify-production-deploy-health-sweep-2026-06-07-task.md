...
Paperclip DRE heartbeat for [LUC-2961](/LUC/issues/LUC-2961), scoped to a
read-only production deploy health sweep. Wake payload had no pending comments
...
Timestamp: `2026-06-07T22:31:41Z` through `2026-06-07T22:32:45Z` UTC.

### Source Ref Snapshot

- Local `HEAD`: `ed0f1aeb0e60392fe553f46d4931f9d9742f6aec`
...
| Resource | Kind | Status | Restart metadata |
| --- | --- | --- | --- |
| `soar-web` | application | `running:unknown` | `restart_count=0` |
| `soar-api` | application | `running:unknown` | `restart_count=0` |
| `workers-backtest` | application | `running:unknown` | `restart_count=0` |
| `workers-execution` | application | `running:unknown` | `restart_count=2`, `last_restart_type=crash`, `last_restart_at=2026-06-06T04:12:15.000000Z` |
| `workers-market-data` | application | `running:unknown` | `restart_count=0` |
| `workers-market-stream` | application | `running:unknown` | `restart_count=0` |
| `postgresql` | standalone-postgresql | `running:healthy` | historical restart metadata visible |
| `redis` | standalone-redis | `running:healthy` | historical restart metadata visible |

Redacted app log/restart projection:

| App | Log pattern summary |
| --- | --- |
| `soar-api` | none of checked patterns |
| `soar-web` | contains `error` string |
| `workers-execution` | contains `worker_heartbeat` |
| `workers-market-stream` | none of checked patterns |

Diagnosis: no public outage is visible. The retained
`workers-execution` crash metadata remains present and was already classified
in [LUC-2594](/LUC/issues/LUC-2594) as
...
- Coolify deployments API did not expose recent deployment rows in this token
  context;
- retained `workers-execution` crash cause remains unknown from accessible
