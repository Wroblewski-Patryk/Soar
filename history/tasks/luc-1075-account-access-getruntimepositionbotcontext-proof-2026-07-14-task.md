# LUC-1075 Account Access getRuntimePositionBotContext Proof

## Header
- ID: LUC-1075
- Title: Account access `getRuntimePositionBotContext` missing-test-link proof
- Task Type: fix
...
- Depends on: none
- Priority: P1
- Module Confidence Rows: Account access / API bots runtime position bot-context repository helper executable proof
- Requirement Rows: not applicable
- Quality Scenario Rows: not applicable
- Risk Rows: app-completion missing-test-link routing for Account access runtime position bot-context repository helper
- Iteration: 1
- Operation Mode: BUILDER
- Mission ID: LUC-1075-ACCOUNT-ACCESS-GETRUNTIMEPOSITIONBOTCONTEXT-PROOF-2026-07-14
- Mission Status: VERIFIED

## Context

`LUC-1075` was dispatched from the Account access app-completion queue to close
...
Close the missing-test-link lane for `getRuntimePositionBotContext` with the
...
- [x] Direct executable proof covers the owned-bot runtime context projection
      request into `prisma.bot.findFirst`.
...
- Tests:
  - `corepack pnpm --filter api exec vitest run src/modules/bots/getRuntimePositionBotContext.repository.test.ts --run --reporter=dot`
- Source truth:
...
- Task summary:
  - added a focused repository proof in
    `getRuntimePositionBotContext.repository.test.ts`, linked the helper
