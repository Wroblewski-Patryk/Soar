...
- Status: VERIFIED, CONTROL-PLANE CLOSURE DELEGATED
- Owner: Ops/Release
- Depends on: none
- Priority: P1
...
## Context
`LUC-1537` asks Ops to confirm the expected Coolify team/workspace selector so
...
## Scope
- Paperclip issue context for `LUC-1537`.
- Coolify read-only API endpoints for teams, current team, configured project,
  configured environments, and configured production environment.
...
- [x] Paperclip parent disposition attempted.
- [x] Parent disposition blocker delegated to recovery checkout owner in `LUC-1542`.
...
- Files changed: this task packet, this evidence packet, project state, task board.
- How tested: read-only Paperclip context and Coolify API readback.
- What is incomplete: explicit `COOLIFY_SOAR_TEAM_ID` / `COOLIFY_TEAM_ID` binding remains absent but is not an active blocker while the selector is documented and project reads succeed. Paperclip parent `LUC-1537` status write is blocked by active recovery checkout ownership outside this actor.
- Next steps: `LUC-1542` recovery owner applies the parent `done` disposition or releases/reassigns the checkout; run resource reconciliation when assigned.
