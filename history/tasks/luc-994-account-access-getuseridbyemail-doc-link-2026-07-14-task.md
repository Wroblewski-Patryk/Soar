...
## Header
- ID: LUC-994
- Title: Prove Account access missing-doc-link for `getUserIdByEmail`
- Task Type: fix
...
- Mission ID: LUC-994-ACCOUNT-ACCESS-GETUSERIDBYEMAIL-DOC-LINK-2026-07-14
...
## Mission Block
- Mission objective: close the docs-owned `missing_doc_link` routing for `getUserIdByEmail`.
- Release objective advanced: project truth now advances to the next real Account access docs gap instead of the close-authority owner-id lookup helper.
...
## Context
`docs/status/project-truth-index.md` routed `getUserIdByEmail` as the first
Account access `missing_doc_link` gap after [LUC-983](/LUC/issues/LUC-983)
closed the neighboring `createBotWithRuntimeSession` helper on 2026-07-13. The
helper already had executable coverage inside
`apps/api/src/modules/bots/bots.runtime-close-authority.route-pack.e2e.test.ts`,
but repo truth did not yet document that owner-id lookup helper in the API bots
module or map the `documents` relation into the generated graph.
...
- Expected product or reliability outcome: future agents can reason about the close-authority owner-id lookup helper from canonical docs and skip this stale gap.
...
- [x] Generated app-completion and project-truth readback no longer route `getUserIdByEmail` as `missing_doc_link`.
...
- Screenshots/logs: `history/artifacts/luc-994-build-architecture-awareness-log.txt`
...
- Permission or ownership checks: no runtime ownership logic changed; this lane only documented the already-tested helper boundary
...
- Issues: project truth first gap was `getUserIdByEmail` as `missing_doc_link`.
...
- Selected task: close the `getUserIdByEmail` doc-link routing
- Priority rationale: first Account access project-truth gap and direct wake scope
- Why other candidates were deferred: `seedTicker`, `registerAndLogin`, and other helper gaps remain separate follow-ups
...
- Implementation notes: documented the runtime close-authority owner-id lookup helper in the API bots module, added the matching CSV relation, and added the scanner `documents` override.
...
- Result: pass; app-completion no longer routes `getUserIdByEmail`, and project truth advanced to the next Account access helper
...
- Refinements made: reused the redirected-log builder pattern already proven in the neighboring route-pack helper closure
...
- Task summary: closed the docs-owned `getUserIdByEmail` gap across the module doc and canonical relation registries, then verified generated truth advanced to the next Account access helper.
...
- What is incomplete: the next docs-owned Account access gap remains `apps/api/src/modules/bots/bots.runtime-close-dca-authority.e2e.test.ts#seedTicker`.
- Next steps: route the next first-gap docs closure for `seedTicker`.
