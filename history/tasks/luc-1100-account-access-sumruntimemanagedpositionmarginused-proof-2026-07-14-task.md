# LUC-1100 Account Access sumRuntimeManagedPositionMarginUsed Proof

## Header
- ID: LUC-1100
- Title: Account access `sumRuntimeManagedPositionMarginUsed` missing-test-link proof
- Task Type: fix
...
- Depends on: none
- Priority: P1
- Module Confidence Rows: Account access / API bots runtime managed-position margin-used repository helper executable proof
- Requirement Rows: not applicable
- Quality Scenario Rows: not applicable
- Risk Rows: app-completion missing-test-link routing for Account access runtime managed-position margin-used repository helper
- Iteration: 1
- Operation Mode: BUILDER
- Mission ID: LUC-1100-ACCOUNT-ACCESS-SUMRUNTIMEMANAGEDPOSITIONMARGINUSED-PROOF-2026-07-14
- Mission Status: VERIFIED

## Context

`LUC-1100` was dispatched from the Account access app-completion queue to close
...
Close the missing-test-link lane for `sumRuntimeManagedPositionMarginUsed` with
...
- Tests:
  - `corepack pnpm --filter api exec vitest run src/modules/bots/sumRuntimeManagedPositionMarginUsed.repository.test.ts --run --reporter=dot`
- Source truth:
...
- Task summary:
  - added a focused repository proof in
    `sumRuntimeManagedPositionMarginUsed.repository.test.ts`, linked the helper
