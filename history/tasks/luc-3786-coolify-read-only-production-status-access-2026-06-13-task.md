...
[LUC-3786](/LUC/issues/LUC-3786) requested Coolify base URL, API token,
project id, and team id bindings so Paperclip can reconcile read-only
production deploy status after pushes.

## Goal

Confirm that required Coolify binding names are present and usable for
authenticated read-only status reads, while preserving the no-secret and
...
6. Update [LUC-3786](/LUC/issues/LUC-3786) to `done` with evidence.
...
- Evidence and issue disposition avoid secrets, raw ids, raw logs, and
  internal URLs.
...
- Do not expose token values, raw configured ids, raw resource ids, internal
  URLs, labels, proxy settings, cookies, credentials, or log bodies.
...
- Residual risk: read-only binding usability is not proof of app readiness,
  protected auth, worker freshness, rollback, restore, SLO, or release
  approval.

## Result Report

- Task summary: Soar Coolify read-only production status access verified at
  `2026-06-13T14:31:33Z` without value disclosure.
...
- Next steps: Ops/Release should monitor or reconcile the visible `soar-api`
  active deployment row if a release lane depends on its completion.
