...
## Goal
Publish a durable no-stall checkpoint for `LUC-116` with explicit blocker owner/action and fail-closed final disposition.
...
- Next steps:
  1. Keep `LUC-116` and `LUC-45` as `blocked` when idle; use `in_progress` only during active reconciliation.
