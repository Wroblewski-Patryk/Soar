# Task Contract - LUC-1587 Confirm Coolify Team Workspace
...
`LUC-1587` asks Ops to confirm the expected Coolify team/workspace selector so
...
1. Read Paperclip heartbeat context for `LUC-1587`.
2. Check Coolify binding names only.
3. Run read-only Coolify API probes for current team, teams list, configured
   Soar project, production environment, and redacted production inventory.
...
- `GET /api/v1/projects/{configured-project-id}/production` returns the known
  eight-resource production topology: six applications, PostgreSQL, and Redis.
...
- Verification:
  ...
  - `GET /api/v1/teams/current` -> pass at `2026-06-02T19:12:30Z`.
...
- Evidence: `history/evidence/luc-1587-coolify-team-workspace-confirmation-2026-06-02.md`.
