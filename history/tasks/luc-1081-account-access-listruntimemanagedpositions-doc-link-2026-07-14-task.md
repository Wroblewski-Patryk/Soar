...
- ID: `LUC-1081`
- Title: Account access `listRuntimeManagedPositions` missing-doc-link closure
- Task Type: `fix`
...
[LUC-1079](/LUC/issues/LUC-1079) closed the helper's focused repository proof.

## Goal

Attach durable module documentation and canonical graph relations for
`listRuntimeManagedPositions` so generated app-completion and project-truth no
...
- Readback:
  `listRuntimeManagedPositions` is no longer a docs-owned first gap. The next
  ...
  as `missing_doc_link`.
- Residual:
  direct proof remains owned by [LUC-1079](/LUC/issues/LUC-1079), and the
