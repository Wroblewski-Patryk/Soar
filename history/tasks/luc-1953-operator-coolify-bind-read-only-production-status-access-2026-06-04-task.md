# LUC-1953 Task Contract - Coolify Read-Only Production Status Access
...
[LUC-1953](/LUC/issues/LUC-1953) asked to bind or validate read-only Coolify
...
- Coolify readback at `2026-06-04T09:48:12Z` resolved project `Soar`,
...
- Application inventory status remains `running:unknown`; PostgreSQL and Redis
  resource classes are present, but this environment projection did not expose
  a non-sensitive status field for them in this read.
- `GET /api/v1/resources` returned `17` visible rows and was not used as release
...
readiness, database health, and protected worker readiness still require
separate release smoke or a release mutation permit when applicable.
