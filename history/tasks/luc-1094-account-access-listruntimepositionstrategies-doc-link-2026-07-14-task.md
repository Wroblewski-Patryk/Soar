# LUC-1094 Account Access listRuntimePositionStrategies Doc-Link Closure

## Context

- ID: `LUC-1094`
- Title: Account access `listRuntimePositionStrategies` missing-doc-link closure
- Task Type: `fix`
...
[LUC-1091](/LUC/issues/LUC-1091) closed the helper's focused repository proof.

## Goal

Attach durable module documentation and canonical graph relations for
`listRuntimePositionStrategies` so generated app-completion and project-truth
...
- Readback:
  `listRuntimePositionStrategies` is no longer a docs-owned first gap. The
  next overall Account access row advances to
  ...
  as `missing_doc_link`.
- Residual:
  direct proof remains owned by [LUC-1091](/LUC/issues/LUC-1091), and the
