...
## Context
`LUC-1548` asks Ops to confirm the expected Coolify team/workspace selector so
...
## Scope
- Paperclip issue context for `LUC-1548`.
...
- Manual checks:
  ...
  - `GET /api/v1/teams/current` -> pass at `2026-06-02T16:34:09Z`, current selector id `0`, name `LuckySparrow`.
...
- Selected task: confirm Coolify team/workspace selector for `LUC-1548`.
- Priority rationale: wrong workspace context can invalidate production evidence.
- Why other candidates were deferred: wake payload scoped this heartbeat to `LUC-1548`.
...
### 6. Self-Review
- Simpler option considered: relying only on previous `LUC-1543` evidence.
...
- Docs updated: this task packet, evidence packet, operations deployment contract, runtime config ledger, project context, task board.
...
- Files changed: this task packet, evidence packet, operations deployment contract, runtime config ledger, project state, task board.
