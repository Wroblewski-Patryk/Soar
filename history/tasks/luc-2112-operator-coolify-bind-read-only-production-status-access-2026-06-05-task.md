# LUC-2112 Operator Coolify Bind Read-Only Production Status Access
...
Paperclip wake payload assigned [LUC-2112](/LUC/issues/LUC-2112) as a critical
...
- Relevant state and operations ledgers updated.
...
Fresh read-only Coolify readback at `2026-06-05T07:23:02Z` confirmed required
...
generic services, and `17` visible global resource rows. Application inventory
status remains `running:unknown`; PostgreSQL reports `running:healthy`; Redis
resource presence is verified but status is `running:unknown` in this redacted
readback.
