# LUC-1585 Confirm Coolify Team Workspace Task

## Context

Paperclip issue `LUC-1585` requested confirmation of the expected Coolify
team/workspace selector for Soar production. The risk is that a wrong Coolify
team context could make Ops inspect or mutate the wrong project. The latest
issue comment was a live-run janitor note about cancelling a duplicate active
lane; this heartbeat used the scoped wake and performed a fresh read-only proof.
...
- Paperclip issue heartbeat context for `LUC-1585`
...
2. Acknowledge the latest issue comment and verify it does not change the
   safe read-only proof path.
...
Verified at `2026-06-02T19:12:54Z`: current Coolify team/workspace selector is
...
contains eight resources: six applications plus PostgreSQL and Redis.
