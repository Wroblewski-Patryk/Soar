# LUC-1575 Reconcile Coolify Resource Inventory - 2026-06-02
...
- [x] The task improves release confidence, not only local file appearance.
...
- Read-only Coolify API probes for current team selector, configured Soar
  project, production environment, production inventory, and global resource
  row count.
...
- Do not store Coolify resource ids or public URLs in evidence.
...
- Manual checks:
  ...
  - `GET /api/v1/teams/current` -> pass at `2026-06-02T18:33:56Z`, id `0`,
    name `LuckySparrow`.
...
- Selected task: `LUC-1575` Coolify resource inventory reconciliation.
...
- Implementation notes: queried Coolify current selector, project, production
  environment, production inventory, and global resource count without printing
  secrets or storing IDs.
...
- Simpler option considered: reuse `LUC-1569` evidence; rejected because this
