...
- Manual checks:
  - `GET /api/issues/LUC-1601/heartbeat-context` -> pass; no first-class blockers.
  ...
  - production environment read -> pass, environment `production`.
  - resource projection -> pass, six applications, one PostgreSQL, one Redis.
...
- Project docs already recorded prior selector proof, but fresh read-only API
  proof was required for this heartbeat.
