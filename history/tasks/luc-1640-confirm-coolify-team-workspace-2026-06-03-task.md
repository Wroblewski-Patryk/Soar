...
- Mission ID: LUC-1640-COOLIFY-TEAM-WORKSPACE-CONFIRMATION-2026-06-03
...
## Scope
- Paperclip issue context: `LUC-1640`
...
## Implementation Plan
1. Read Paperclip heartbeat context for `LUC-1640`.
...
- Manual/API checks:
  - `GET /api/issues/LUC-1640/heartbeat-context` -> pass; issue read back as
    `blocked` with zero first-class blockers.
  - Names-only env presence check -> pass.
  - `GET /api/v1/teams/current` -> pass, selector id `0`, name
    `LuckySparrow`.
...
- Files changed: evidence, task packet, operations ledger/context.
...
- What is incomplete: explicit `COOLIFY_SOAR_TEAM_ID` or `COOLIFY_TEAM_ID`
  binding is still absent, but not a blocker while current selector readback and
