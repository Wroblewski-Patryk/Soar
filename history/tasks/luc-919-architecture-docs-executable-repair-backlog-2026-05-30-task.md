...
## Definition of Done
- [x] Current control map exists for `LUC-919` with all `ARB-001..ARB-008` rows.
...
2. Publish refreshed `LUC-919` control map artifact.
3. Synchronize source-of-truth context files with final disposition and blockers.

## Acceptance Criteria
- `history/plans/luc-919-architecture-repair-backlog-control-map-2026-05-30.md` exists and references canonical backlog/execution artifacts.
- Control map reflects `DEC-ARB-001` and `DEC-ARB-002` as decision-gated rows and keeps `ARB-006` as the active blocker.
- `TASK_BOARD` and `PROJECT_STATE` record `LUC-919` as `done` planning lane.

## Validation Evidence
- `rg -n "LUC-919|DEC-ARB-001|DEC-ARB-002|ARB-006|control_map_refreshed" history/plans/luc-919-architecture-repair-backlog-control-map-2026-05-30.md history/tasks/luc-919-architecture-docs-executable-repair-backlog-2026-05-30-task.md .codex/context/TASK_BOARD.md .codex/context/PROJECT_STATE.md`
...
- Continuation heartbeat (wake `issue_continuation_needed`) reconciliation:
  - revalidated the same control-map truth for `ARB-001..ARB-008`,
  - confirmed `ARB-006` remains the only active blocker and no new unblock evidence exists,
  - kept final disposition `done` for this planning/state lane.
- Files changed:
