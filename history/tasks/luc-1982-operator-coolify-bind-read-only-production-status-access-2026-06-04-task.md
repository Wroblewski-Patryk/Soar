# LUC-1982 Task Contract - Coolify Read-Only Production Status Access
...
[LUC-1982](/LUC/issues/LUC-1982) asked to bind or validate read-only Coolify
...
- Coolify readback at `2026-06-04T13:05:16Z` resolved project `Soar`,
...
- `GET /api/v1/resources` returned `1` visible row in this least-privilege
  runner session and was not used as release authority for this checkpoint.
- `pnpm run ops:coolify-stack:env-check:test` passed (`8/8`).
- Paperclip heartbeat context returned the scoped issue as `in_progress`,
  priority `critical`, with zero first-class blockers and no comments.
