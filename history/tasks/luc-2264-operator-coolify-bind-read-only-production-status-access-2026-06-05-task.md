# LUC-2264 Operator Coolify Bind Read-Only Production Status Access

Date: 2026-06-05
Stage: verification
Owner: Ops Release Lead
Issue: [LUC-2264](/LUC/issues/LUC-2264)

## Context

Paperclip wake payload assigned [LUC-2264](/LUC/issues/LUC-2264) as a critical
Ops issue to bind or verify Coolify read-only production status access.
Checkout was already claimed by the harness and was not repeated. The issue had
no pending comments, so no comment-driven scope change was required.
...
- Source-of-truth refresh for current project state, task board, service
  topology, runtime ledger, and Coolify deployment contract.
...
1. Consume the scoped wake for [LUC-2264](/LUC/issues/LUC-2264).
...
5. Refresh relevant source-of-truth docs.
...
- Relevant source-of-truth docs updated.
- Relevant verification command passes.
- No production mutation occurred.
- Source-control closure decision recorded.
...
- Architecture source reviewed: Ops role contract, release safety contract, and
  existing Coolify service topology docs.
...
Fresh read-only Coolify readback at `2026-06-05T18:53:12Z` confirmed required
...
report `running:healthy` from production environment readback. The teams list
endpoint returned `0` visible team rows in this runner, but current selector
and project-scoped reads succeeded.
