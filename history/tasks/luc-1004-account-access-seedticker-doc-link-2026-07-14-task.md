...
## Header
- ID: LUC-1004
- Title: Prove Account access missing-doc-link for `seedTicker`
- Task Type: fix
...
- Depends on: none
- Priority: P1
- Module Confidence Rows: Account access / API bots runtime close DCA-authority docs truth
- Requirement Rows: not applicable
- Quality Scenario Rows: not applicable
- Risk Rows: app-completion missing-doc-link routing for Account access close-price helper
- Iteration: 1
- Operation Mode: BUILDER
- Mission ID: LUC-1004-ACCOUNT-ACCESS-SEEDTICKER-DOC-LINK-2026-07-14
...
## Mission Block
- Mission objective: close the docs-owned `missing_doc_link` routing for `seedTicker`.
- Release objective advanced: project truth now advances past the DCA close-price ticker helper to the next real Account access docs gap.
...
| QA/Test | coordinator | existing DCA-authority proof file | focused readback only | no new proof required | generator chain | COMPLETE |
...
## Context
`docs/status/project-truth-index.md` routed `seedTicker` as the first Account
access `missing_doc_link` gap after [LUC-994](/LUC/issues/LUC-994) closed the
neighboring `getUserIdByEmail` helper on 2026-07-14. The helper already had
executable coverage inside
`apps/api/src/modules/bots/bots.runtime-close-dca-authority.e2e.test.ts`, but
repo truth did not yet document that trusted runtime ticker-seeding helper in
the API bots module or map the `documents` relation into the generated graph.
...
- Expected product or reliability outcome: future agents can reason about the DCA close-price ticker seeding helper from canonical docs and skip this stale gap.
...
- [x] Generated app-completion and project-truth readback no longer route `seedTicker` as `missing_doc_link`.
...
- Screenshots/logs: `history/artifacts/luc-1004-build-architecture-awareness-log.txt`
...
- Permission or ownership checks: no runtime ownership logic changed; this lane only documented the already-tested trusted-price helper boundary
...
- Residual risk: adjacent Account access helper doc gaps still remain after this slice

## Autonomous Loop Evidence

### 1. Analyze Current State
- Issues: project truth first gap was `seedTicker` as `missing_doc_link`.
- Gaps: no durable doc relation existed for the helper.
- Inconsistencies: executable proof existed in the DCA-authority e2e file, docs relation did not.
- Architecture constraints: use canonical module docs and graph registries only.

### 1a. Bootstrap Missing Project Knowledge
- Bootstrap needed: no
- Missing or template-like files: none
- Sources scanned: `AGENTS.md`, `docs/status/project-truth-index.md`, `docs/status/app-completion-index.md`, `docs/modules/api-bots.md`, `documentation-links.csv`, `scanner-overrides.json`, `apps/api/src/modules/bots/bots.runtime-close-dca-authority.e2e.test.ts`
- Rows created or corrected: scoped module-doc row plus matching `documents` relations
- Assumptions recorded: safe to treat the helper as docs-owned because the DCA close-authority proof already existed and the wake named only a missing doc link
...
- Selected task: close the `seedTicker` doc-link routing
- Priority rationale: first Account access project-truth gap and direct wake scope
- Why other candidates were deferred: `registerAndLogin` and other helper gaps remain separate follow-ups
...
- Implementation notes: documented the runtime close DCA-authority trusted-price helper in the API bots module, added the matching CSV relation, and added the scanner `documents` override.
...
- Result: pass; app-completion no longer routes `seedTicker`, and project truth advanced to the next Account access helper
...
- Refinements made: reused the redirected-log builder pattern already proven in the neighboring runtime close helper closures
...
- Task summary: closed the docs-owned `seedTicker` gap across the module doc and canonical relation registries, then verified generated truth advanced to the next Account access helper.
...
- What is incomplete: the next docs-owned Account access gap remains `apps/api/src/modules/bots/bots.subscription-entitlements.e2e.test.ts#registerAndLogin`.
- Next steps: route the next first-gap docs closure for the subscription-entitlements `registerAndLogin` helper.
