...
post-push production status reconciliation. Prior 2026-06-02 evidence showed
project-scoped access worked, while optional team-id bindings remain absent in
the runner. This heartbeat verified the current binding state again for the
new issue.
...
- Manual checks:
  ...
  - `GET /api/v1/teams/current` returned `200`, selector id `0`, name
    `LuckySparrow`;
  - `GET /api/v1/projects/{COOLIFY_SOAR_PROJECT_ID}` returned `200` and
    resolved to `Soar`;
  - `GET /api/v1/projects/{COOLIFY_SOAR_PROJECT_ID}/environments` returned
    `200` and included `production`;
  - `GET /api/v1/projects/{COOLIFY_SOAR_PROJECT_ID}/production` returned
    `200` and eight production resources;
...
- Architecture source reviewed: `docs/operations/coolify-vps-deployment-contract.md`,
  `docs/operations/runtime-config-ledger.csv`, `docs/operations/service-topology.md`.
...
- Next steps: close LUC-1579 as done; use project/environment/resource
