...
- ID: `LUC-1073`
- Title: Account access `countRuntimeManagedPositions` missing-doc-link closure
- Task Type: `fix`
...
[LUC-1071](/LUC/issues/LUC-1071) closed the helper's focused repository proof.

## Goal

Attach durable module documentation and canonical graph relations for
`countRuntimeManagedPositions` so generated app-completion and project-truth no
...
- Readback:
  `countRuntimeManagedPositions` is no longer a docs-owned first gap. The next
  Account access front row advances to
  `apps/api/src/modules/bots/runtimeSessionPositionsRead.repository.ts#getRuntimePositionBotContext`
  as `missing_test_link`, while the docs-owned queue advances away from the
  scoped helper.
- Residual:
  direct proof remains owned by [LUC-1071](/LUC/issues/LUC-1071), and the
