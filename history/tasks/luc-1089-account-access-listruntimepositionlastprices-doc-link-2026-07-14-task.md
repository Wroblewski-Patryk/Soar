# LUC-1089 Account Access listRuntimePositionLastPrices Doc-Link Closure

## Context

- ID: `LUC-1089`
- Title: Account access `listRuntimePositionLastPrices` missing-doc-link closure
- Task Type: `fix`
...
[LUC-1087](/LUC/issues/LUC-1087) closed the helper's focused repository proof.

## Goal

Attach durable module documentation and canonical graph relations for
`listRuntimePositionLastPrices` so generated app-completion and project-truth
...
- Readback:
  `listRuntimePositionLastPrices` is no longer a docs-owned first gap. The
  next overall Account access front row advances to
  ...
  as `missing_doc_link`.
- Residual:
  direct proof remains owned by [LUC-1087](/LUC/issues/LUC-1087), and the
