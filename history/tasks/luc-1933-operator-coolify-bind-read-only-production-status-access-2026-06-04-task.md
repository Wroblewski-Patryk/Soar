# LUC-1933 Task Contract - Coolify Read-Only Production Status Access
...
[LUC-1933](/LUC/issues/LUC-1933) asked to bind or validate read-only Coolify
production status access for Soar so Paperclip can reconcile whether Coolify
can expose production project, environment, and resource status after source
changes.
...
- Run authenticated read-only Coolify API reads for current team, project,
  environments, production environment resources, and global resource count.
...
- Read-only Coolify calls resolve selector, Soar project, production
  environment, and redacted resource inventory.
...
- Optional team binding names remain absent:
  ...
  current-team and project-scoped readbacks succeeded.
- Coolify readback at `2026-06-04T08:48:23Z` resolved current selector id `0`,
  name `LuckySparrow`, project `Soar`, environment `production`, and eight
  canonical production resources: `soar-api`, `soar-web`,
...
- `GET /api/v1/resources` returned `17` visible rows.
...
readiness and protected worker readiness still require separate release smoke
or a release mutation permit when applicable.
