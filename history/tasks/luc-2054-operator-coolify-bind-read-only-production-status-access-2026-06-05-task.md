# LUC-2054 Task Contract - Coolify Read-Only Production Status Access
...
[LUC-2054](/LUC/issues/LUC-2054) asked to bind or validate read-only Coolify
...
- Coolify readback at `2026-06-04T23:40:30Z` resolved project `Soar`,
...
- Wake payload scoped this heartbeat to [LUC-2054](/LUC/issues/LUC-2054),
  status `in_progress`, priority `critical`, with no pending comments and
  `fallbackFetchNeeded=false`.
- Local probe retries: unsupported PowerShell `??` syntax and one invalid
  empty pipeline failed before any Coolify API request; compatible reruns
  succeeded with no secret output.
