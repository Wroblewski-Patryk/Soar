# LUC-1085 Account Access listRuntimeOpenOrders Doc-Link Closure

## Context

- ID: `LUC-1085`
- Title: Account access `listRuntimeOpenOrders` missing-doc-link closure
- Task Type: `fix`
...
[LUC-1083](/LUC/issues/LUC-1083) closed the helper's focused repository proof.

## Goal

Attach durable module documentation and canonical graph relations for
`listRuntimeOpenOrders` so generated app-completion and project-truth no
...
- Readback:
  `listRuntimeOpenOrders` is no longer a docs-owned first gap. The next
  ...
  as `missing_doc_link`.
- Residual:
  direct proof remains owned by [LUC-1083](/LUC/issues/LUC-1083), and the
