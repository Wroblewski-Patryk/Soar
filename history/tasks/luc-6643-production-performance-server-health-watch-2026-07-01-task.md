...
- Mission ID: LUC-6643-PRODUCTION-PERFORMANCE-SERVER-HEALTH-WATCH-2026-07-01
...
## Context
LUC-6643 is a DRE-scoped production performance and server health watch. The
...
## Validation Evidence
- `GET /api/issues/LUC-6643/heartbeat-context` -> `200`; issue status
  `in_progress`.
- `GET /api/issues/LUC-6331/heartbeat-context` -> `200`; blocker status
  `blocked`.
...
- Runtime freshness with `DEPLOY_FRESHNESS_*` mapped from existing `SMOKE_AUTH_*` names -> `PASS`; worker/market age `17734 ms`, runtime signal lag `0 ms`, running sessions `5`, stale sessions `0`.
