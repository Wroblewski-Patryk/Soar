...
## Header
- ID: LUC-789
- Title: Account access `resolveSessionWindowEnd` missing-doc-link closure
- Task Type: fix
...
- Mission ID: LUC-789-ACCOUNT-ACCESS-RESOLVESESSIONWINDOWEND-DOC-LINK-2026-07-12
- Mission Status: VERIFIED

## Context

`docs/status/project-truth-index.md` routed the first Account access gap to
`apps/api/src/modules/bots/botOwnership.service.ts#resolveSessionWindowEnd` as
`missing_doc_link`. The task was to prove whether the gap was a real docs miss
or a stale/generated-state problem.

## Goal

Prove whether `resolveSessionWindowEnd` is truly undocumented or whether the
repo already has the needed source-truth inputs and the generated
...
- [x] `docs/modules/api-bots.md` documents
      `botOwnership.service.ts#resolveSessionWindowEnd`.
- [x] Documentation-link inputs and scanner overrides include the scoped helper.
- [x] Generated readback proves whether the helper still remains
...
- Issues:
  - project truth routed `resolveSessionWindowEnd` as the first Account access
    `missing_doc_link` row.
- Gaps:
  - the generated queue claimed the helper was missing a doc-link.
- Inconsistencies:
  - the same file already had adjacent ownership-helper classifications, so the
    row could be either a missing docs input or an ingestion miss.
- Architecture constraints:
  - repair through source-of-truth docs and generators only.

### 2. Select One Priority Mission Objective
- Selected task:
  - prove the scoped `resolveSessionWindowEnd` missing-doc-link row.
- Priority rationale:
  - it was the current first Account access docs-owned gap in project truth.
...
- Implementation notes:
  - added the helper classification, doc-link registry row, and `documents`
    relation override.
...
- Scalability assessment:
  - follows the same bounded doc-link closure pattern as recent DSM rows.
...
- Task summary:
  - proved that `resolveSessionWindowEnd` now has local docs inputs, but the
    generated graph/status chain still fails to ingest them, so the
