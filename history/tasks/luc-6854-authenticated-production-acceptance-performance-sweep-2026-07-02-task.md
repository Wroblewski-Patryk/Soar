...
- Mission ID: `LUC-6854-AUTHENTICATED-PRODUCTION-ACCEPTANCE-PERFORMANCE-SWEEP-2026-07-02`
...
## Constraints
- Do not print or persist credentials, tokens, cookies, secrets, protected
  response bodies, or private account data.
- Do not mutate production config, exchange/payment settings, subscription
  state, DB/Redis data, orders, positions, or live-trading state.
- Do not commit, push, deploy, restart, or roll back from this QA lane.
- Reuse existing production smoke and audit scripts.

## Forbidden
- Secret/account value readback.
- Production mutation outside an approved Ops/Security lane.
- Duplicate broad QA backlog creation while [LUC-6331](/LUC/issues/LUC-6331)
  owns the restoration blocker.
...
- Evidence:
  - `history/evidence/luc-6854-authenticated-production-acceptance-performance-sweep-2026-07-02.md`
  - `history/evidence/luc-6854-prod-ui-module-clickthrough-2026-07-02.md`
  - `history/artifacts/luc-6854-prod-ui-module-clickthrough-2026-07-02.json`

## Implementation Plan
1. Read the issue heartbeat context and confirm no new comments or blockers.
2. Run deploy smoke against production Web/API.
3. Run protected worker readiness with environment-only auth bindings.
4. Run runtime freshness and rollback guard.
5. Run production UI clickthrough with redacted audit-login mapping.
6. Attempt auth-session proof only far enough to prove the build-info
   precondition; record the blocker if build-info is unavailable.
7. Block the issue on the existing Ops restoration lane instead of duplicating
   a broad QA backlog.
...
## Validation Evidence
- `pnpm run -s ops:deploy:smoke -- --base-url https://api.soar.luckysparrow.ch --web-base-url https://soar.luckysparrow.ch --no-workers` -> FAIL, Web/build-info `503`
- `pnpm run -s ops:deploy:smoke -- --base-url https://api.soar.luckysparrow.ch --web-base-url https://soar.luckysparrow.ch` -> FAIL, Web/build-info/workers ready `503`
- `pnpm run -s ops:deploy:runtime-freshness` -> PASS
- `pnpm run -s ops:deploy:rollback-guard` -> FAIL with `shouldRollback=true`
- `pnpm run -s ops:ui:prod-clickthrough -- --today 2026-07-02 --output-json history/artifacts/luc-6854-prod-ui-module-clickthrough-2026-07-02.json --output-md history/evidence/luc-6854-prod-ui-module-clickthrough-2026-07-02.md` -> FAIL, all audited Web route groups `503`
- `pnpm run -s ops:prod-auth:proof -- --today 2026-07-02 --output-json history/artifacts/luc-6854-prod-auth-session-browser-proof-2026-07-02.json --output-md history/evidence/luc-6854-prod-auth-session-browser-proof-2026-07-02.md --i-understand-production-auth-proof` -> FAIL before browser proof because build-info returned `503`
- Manual checks: Paperclip heartbeat-context readback for [LUC-6854](/LUC/issues/LUC-6854) returned `200`.
...
- Data classification: production auth route metadata only; no protected row
  payloads or private response bodies preserved.
- Trust boundaries: production Web/API, protected workers readiness, audit-login
  secret refs.
- Permission or ownership checks: QVE read-only verification lane only; Ops owns
  restoration/mutation.
- Secret handling: values never printed; only script-level `login:present`
  status recorded.
- Fail-closed behavior: acceptance blocked when Web/build-info and worker
  readiness are unavailable.
- Residual risk: full authenticated journey proof is still pending after
...
- Files changed:
  ...
  - `history/evidence/luc-6854-prod-ui-module-clickthrough-2026-07-02.md`
  - `history/artifacts/luc-6854-prod-ui-module-clickthrough-2026-07-02.json`
  - `history/tasks/luc-6854-authenticated-production-acceptance-performance-sweep-2026-07-02-task.md`
