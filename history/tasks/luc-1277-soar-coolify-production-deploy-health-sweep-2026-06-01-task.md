# LUC-1277 - [Soar] Coolify production deploy health sweep (2026-06-01)

## Context
Critical Ops Release Lead heartbeat (`issue_assigned`) requiring fresh production deploy-health truth for canonical Soar hosts.
...
- Added one separate read-only failed-deploy diagnosis slice:
  - `node scripts/checkPostDeployRuntimeFreshness.mjs --base-url https://api.soar.luckysparrow.ch`
  - Result: HTTP 401 (fail-closed without protected credentials)
- No production mutation executed.
- Disposition remains `done` for this issue scope.
