...
- ID: `LUC-1050`
- Title: Account access `resolveClosedResult` missing-doc-link closure
- Stage: `verification`
- Mission:
  `LUC-1050-ACCOUNT-ACCESS-RESOLVECLOSEDRESULT-DOC-LINK-2026-07-14`
- Source gap:
  ...
  the preceding `selectRuntimeOpenOrders` proof closure.

## Goal

Attach durable module documentation and graph relations for
`resolveClosedResult` so generated app-completion and project-truth no longer
classify it as `missing_doc_link`.
...
- Validation:
  `build-architecture-awareness-index.mjs` PASS (`10958` entities /
  `36290` relations / `relationOverridesApplied=42`);
  `pnpm run architecture:graph:drift:strict` PASS (`863/863`, `0` missing);
  `build-app-completion-index.mjs` PASS (`missingDocLink=1983`);
...
- Readback:
  `resolveClosedResult` is no longer a docs-owned first gap.
