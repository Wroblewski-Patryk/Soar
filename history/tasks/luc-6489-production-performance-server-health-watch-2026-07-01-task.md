...
- Status: BLOCKED
- Owner: Soar Product Manager / Ops coordination
...
- Iteration: 2026-07-01 Paperclip local repair heartbeat
- Operation Mode: BUILDER
- Mission ID: `LUC-6489-PRODUCTION-PERFORMANCE-SERVER-HEALTH-WATCH-2026-07-01`
- Mission Status: BLOCKED

## Mission Block
- Mission objective: refresh read-only production health and performance watch evidence for [LUC-6489](/LUC/issues/LUC-6489).
...
[LUC-6489](/LUC/issues/LUC-6489) is the recurring production performance and
server-health watch. The latest comment selected an autonomous local
repair/source-control lane and explicitly forbade push, deploy, production
restart, protected smoke/live account mutation, and secret disclosure until
protected gate evidence exists.

## Goal

Produce current, read-only production health evidence and give
[LUC-6489](/LUC/issues/LUC-6489) a clear disposition with source-control
closure.
...
1. Acknowledge the local repair/source-control wake comment.
2. Read issue heartbeat context and current Soar state.
3. Run read-only deploy smoke, runtime freshness, rollback guard, public timing, and Coolify projection.
4. Persist LUC-6489 evidence and task notes.
5. Update state/context with the blocked production watch result.
6. Close the issue as blocked on the existing restoration owner path.
...
- Decisions made: [LUC-6489](/LUC/issues/LUC-6489) should be blocked, not done.
- Commit/no-commit decision: not committed because unrelated dirty worktree state predates this heartbeat; push/deploy forbidden by wake.
