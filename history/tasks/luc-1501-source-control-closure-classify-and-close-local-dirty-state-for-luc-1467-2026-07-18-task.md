...
## Header
- ID: LUC-1501
- Title: Classify and close local dirty state for LUC-1467
- Task Type: release
...
- Mission ID: LUC-1501-SOURCE-CONTROL-CLOSURE-LUC-1467-2026-07-18
- Mission Status: VERIFIED

## Context
`LUC-1467` consumed a resume-delta wake for the blocked `LUC-1438` flow and
left a small local state/evidence packet in the Soar repo. This sidecar lane
exists only to classify that dirty state, preserve it coherently, and avoid
mixing it with unrelated source control work.

## Goal
Close the current `LUC-1467` local state/evidence packet with one reversible
local commit if the worktree contains no unrelated ownership.
...
## Definition of Done
- [x] The dirty worktree is classified for `LUC-1467` ownership and safety.
- [x] A `LUC-1501` task/evidence/closeout packet records the closure decision.
- [x] One local commit closes the coherent `LUC-1467` state/history bundle.
...
## Mission Block
- Mission objective:
  classify the current local dirty packet left by `LUC-1467` and close it as
...
- Manual checks:
  ...
  `LUC-1467` evidence file and no runtime/product paths.
- Screenshots/logs:
  not applicable.
- High-risk checks:
...
- Issues:
  `LUC-1467` was functionally complete for its resume-delta readback but still
  owned a local dirty packet.
- Gaps:
...
- Selected task:
  classify and close the `LUC-1467` dirty state.
- Priority rationale:
...
- Files or surfaces to modify:
  ...
  `history/tasks/...LUC-1501...`,
  `history/evidence/...LUC-1501...`,
  `history/artifacts/...LUC-1501...`.
- Edge cases:
