...
## Goal
Publish a durable no-stall checkpoint for `LUC-167` with explicit blocker
...
- What is incomplete:
  - `LUC-47`: missing temp-domain expected-SHA deploy smoke/readiness + worker
    readiness packet.
- Next steps:
  1. Keep `LUC-167` and `LUC-45` as `blocked` when idle; use `in_progress`
...
- Wake acknowledgment: inline recovery payload introduced no pending human
