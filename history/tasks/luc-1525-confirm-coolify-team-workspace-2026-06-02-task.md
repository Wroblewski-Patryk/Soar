...
## Context
`LUC-1525` asks Ops to confirm the expected Coolify team/workspace selector so
...
## Scope
- Paperclip issue context for `LUC-1525`.
...
- Manual checks:
  - `GET /api/issues/LUC-1525/heartbeat-context` -> pass.
  - names-only env binding check -> pass without printing values.
  - `GET /api/v1/teams` -> pass, two teams visible.
  - `GET /api/v1/teams/current` -> pass at `2026-06-02T15:09:21Z`, id `0`, name `LuckySparrow`.
...
- Files changed: this task packet, this evidence packet, operations deployment contract, runtime config ledger.
