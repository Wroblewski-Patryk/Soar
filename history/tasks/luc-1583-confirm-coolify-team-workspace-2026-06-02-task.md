...
- Source-of-truth updates in `.codex/context/TASK_BOARD.md` and
  `docs/operations/coolify-vps-deployment-contract.md`.
- Evidence artifact under `history/evidence/`.

## Implementation Plan

1. Read the issue context and Ops role constraints.
...
- Manual checks:
  ...
  - resource projection -> pass, 6 applications, 1 PostgreSQL, 1 Redis.
...
- Issue was blocked for missing expected selector.
- Project docs already recorded prior selector proof.
- Live read-only API proof was required before closure.
...
- Updated task board, operations contract, and evidence artifact.
...
- Simpler option considered: using prior docs only. Rejected because the issue
  asked for confirmation and the heartbeat was actionable.
