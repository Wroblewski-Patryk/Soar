...
[LUC-3795](/LUC/issues/LUC-3795) asked for Coolify base URL, token, project id,
and team id bindings so Paperclip can reconcile read-only production deploy
status after pushes. The assignment comment scoped SPM to known-state takeover,
evidence collection, and protected production gates.
...
2. Review latest adjacent Security/Ops Coolify evidence.
...
6. Update [LUC-3795](/LUC/issues/LUC-3795) to `done` with evidence.
...
- Task summary: Soar Coolify read-only production status access verified at
  `2026-06-13T16:42:52Z` without value disclosure.
