# LUC-1100 Evidence - Account access `sumRuntimeManagedPositionMarginUsed` proof
...
- added
  ...
  `sumRuntimeManagedPositionMarginUsed` forwards the scoped `where` clause into
  `prisma.position.aggregate`, requests the exact `_sum.marginUsed` selection,
  and returns the aggregate payload unchanged;
- added a direct `priority-test-links.csv` row for
  `runtimeSessionPositionsRead.repository.ts#sumRuntimeManagedPositionMarginUsed`
  ->
  `sumRuntimeManagedPositionMarginUsed.repository.test.ts`;
...
- `app-completion-index.json` no longer reports
  `sumRuntimeManagedPositionMarginUsed` as `missing_test_link`.
