# LUC-1530 Coolify Resource Inventory Reconciliation Evidence
...
- Issue: `LUC-1530` `[Ops][Soar] Reconcile Coolify resource inventory`
...
| Check | Result |
| --- | --- |
| Paperclip heartbeat context `GET /api/issues/LUC-1530/heartbeat-context` | pass |
...
Fresh redacted readback timestamp: `2026-06-02T15:33:40Z`.
...
- The first environment-list projection used the wrong response-shape assumption and returned `productionPresent=false`; the follow-up names-only read of the same endpoint confirmed `production` is present.
