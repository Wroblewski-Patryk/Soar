# LUC-1083 Evidence - Account access `listRuntimeOpenOrders` proof
...
- added `apps/api/src/modules/bots/listRuntimeOpenOrders.repository.test.ts`
  with a focused no-DB assertion that `listRuntimeOpenOrders` forwards the
  scoped `where` clause and `limit` into `prisma.order.findMany`, preserves
  canonical runtime ordering and projection selection, and returns the payload
  unchanged;
- added a direct `priority-test-links.csv` row for
  `runtimeSessionPositionsRead.repository.ts#listRuntimeOpenOrders` ->
  `listRuntimeOpenOrders.repository.test.ts`;
...
- `app-completion-index.json` no longer reports `listRuntimeOpenOrders` as
  `missing_test_link`.
