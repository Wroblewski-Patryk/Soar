# LUC-3796 Coolify Resource Inventory Reconciliation

- Checked at: 2026-06-13T17:15:08Z
- Owner: 11 SPM (Soar Product Manager)
- Mode: read-only Coolify API projection
- Scope: Soar project, production environment
- Process: operations inventory reconciliation

## Result

Status: verified read-only.

[LUC-3796](/LUC/issues/LUC-3796) reconciled the current Soar production Coolify
resource inventory using the already-present read-only bindings. The
authoritative deployment scope remains:
...
| `workers-execution` | application | `running:unknown` | no | `HEAD` | `/apps/api/Dockerfile.worker.execution` | 0 |
...
| Visible global deployment rows | 1 |

## Deploy Status

- Read-only status access works for the Soar production Coolify hierarchy.
- `GET /api/v1/deployments` showed one visible `soar-api` deployment row with
  status `in_progress` and short commit `9f61eb9781c3`.
...
- A visible `soar-api` deployment row was still `in_progress` at readback time;
  this inventory did not wait for completion and did not inspect raw logs.
- Redis and PostgreSQL report `running:healthy`, but historic restart counts
  remain visible.
- Auto-redeploy cannot be proven from this inventory alone; it only records the
  deployment row visible during this readback.
...
| Deploy queue readback | pass; `1` visible deployment row |
