# LUC-2269 Coolify Read-Only Production Status Access Evidence
...
At `2026-06-05T19:23:32Z`, authenticated read-only Coolify API calls confirmed
...
resource classes, branch names, status summaries, and public-FQDN presence. No
secret values, raw resource identifiers, generated database suffixes, cookies,
tokens, labels, internal URLs, or screenshots were stored.

## Wake Context

- Issue: [LUC-2269](/LUC/issues/LUC-2269)
- Wake reason: `issue_assigned`
- Pending comments: `0/0`
- Fallback fetch needed: `no`
- Latest comment impact: none; the issue had no comments, so the heartbeat
  proceeded directly to the requested Coolify read-only binding proof.
...
- No Coolify token, secret value, cookie, database URL, generated database
  suffix, raw resource id, internal connection URL, or screenshot was stored.
...
- Team selector bindings present by name without value disclosure:
  `COOLIFY_SOAR_TEAM_ID` and `COOLIFY_TEAM_ID`.
- The configured selector resolves to `LuckySparrow`; one team is visible in
  this runner.
...
| Coolify global resources list `GET /api/v1/resources` | pass; `17` visible rows; not used as release authority |
| Data-service production status projection | pass; PostgreSQL and Redis report `running:healthy` |
| `pnpm run ops:coolify-stack:env-check:test` | pass; `8/8` node test subtests passed |

## Redacted Production Status Inventory

| Resource | Coolify type | Inventory status | Branch | Public FQDN |
| --- | --- | --- | --- | --- |
...
class. All expected resource names are present.
