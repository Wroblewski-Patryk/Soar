# LUC-1087 Evidence - Account access `listRuntimePositionLastPrices` proof
...
- added `apps/api/src/modules/bots/listRuntimePositionLastPrices.repository.test.ts`
  with a focused no-DB assertion that `listRuntimePositionLastPrices`
  forwards the scoped `where` clause into
  `prisma.botRuntimeSymbolStat.findMany`, preserves the exact projection
  selection, and returns the payload unchanged;
- added a direct `priority-test-links.csv` row for
  `runtimeSessionPositionsRead.repository.ts#listRuntimePositionLastPrices` ->
  `listRuntimePositionLastPrices.repository.test.ts`;
...
- `app-completion-index.json` no longer reports
  `listRuntimePositionLastPrices` as `missing_test_link`.
