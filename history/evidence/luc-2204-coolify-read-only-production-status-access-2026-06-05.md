# LUC-2204 Coolify Read-Only Production Status Access Evidence
...
At `2026-06-05T12:39:24Z`, authenticated read-only Coolify API calls confirmed
...
- Issue: [LUC-2204](/LUC/issues/LUC-2204)
...
- Optional team binding absent in this runner: `COOLIFY_SOAR_TEAM_ID` /
  `COOLIFY_TEAM_ID`.
- Optional team binding absence is not an active blocker because the current
  selector resolves to `LuckySparrow` and project-scoped readbacks succeeded.
...
| Coolify teams read `GET /api/v1/teams` | pass; `2` teams visible in this runner |
