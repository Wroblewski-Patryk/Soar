# LUC-2054 Coolify Read-Only Production Status Access Evidence
...
At `2026-06-04T23:40:30Z`, authenticated read-only Coolify API calls confirmed
...
- Issue: [LUC-2054](/LUC/issues/LUC-2054)
...
| Redis object-shape readback | pass; environment payload uses `redis`, not `redises`; redacted Redis projection reports `running:healthy` |
...
- The first local PowerShell probe used unsupported `??` syntax and failed
  before any Coolify API request.
- A later Redis narrowing probe had an invalid empty pipeline and failed before
  any Coolify API request.
- Compatible reruns succeeded without printing secret values or raw Coolify
  resource objects.
