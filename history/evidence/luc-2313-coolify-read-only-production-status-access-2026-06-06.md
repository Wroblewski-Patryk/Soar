# LUC-2313 Coolify Read-Only Production Status Access Evidence
...
[LUC-2313](/LUC/issues/LUC-2313) was handled as a narrow Ops verification lane.
...
| Files added | `history/tasks/luc-2313-operator-coolify-bind-read-only-production-status-access-2026-06-06-task.md`; `history/evidence/luc-2313-coolify-read-only-production-status-access-2026-06-06.md` |
...
| Visible teams endpoint | `1` row visible in this runner; name not returned by this endpoint shape |
...
| Authenticated read-only Coolify projection | PASS at `2026-06-05T22:14:35Z`; project, production environment label, and canonical resource counts resolved |
| `pnpm run ops:coolify-stack:env-check:test` | PASS; `8/8` node test subtests |
| `git status --short` | Dirty tree contains unrelated state, backend runtime, and prior evidence/task paths; [LUC-2313](/LUC/issues/LUC-2313) closure does not stage or modify those unrelated paths |
...
Commit decision: not committed in this heartbeat because the repository already
contains substantial unrelated dirty state from other active lanes. The durable
output is limited to the [LUC-2313](/LUC/issues/LUC-2313) task/evidence files
and task-board entry.
