# LUC-2264 Coolify Read-Only Production Status Access Evidence
...
At `2026-06-05T18:53:12Z`, authenticated read-only Coolify API calls confirmed
...
- Present by name without value disclosure: `COOLIFY_BASE_URL`,
  ...
  `COOLIFY_SOAR_API_APP_ID`, `COOLIFY_SOAR_TEAM_ID`, and
  `COOLIFY_TEAM_ID`.
- Legacy single-resource alias `COOLIFY_SOAR_APP_ID` is absent and was not
  needed for this proof.
- Current selector resolves to `LuckySparrow`; the teams list endpoint returned
  `0` visible team rows in this runner, but project-scoped reads succeeded.
...
| Coolify teams read `GET /api/v1/teams` | pass; `0` team rows visible in this runner |
