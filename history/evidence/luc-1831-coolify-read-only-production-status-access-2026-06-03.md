# LUC-1831 Coolify Read-Only Production Status Access Evidence
...
At `2026-06-03T18:04:01Z`, authenticated read-only Coolify API calls
confirmed that this runner can read the configured Soar production status
scope. The proof used only binding names, counts, project/environment labels,
resource names, resource status summaries, and public-FQDN booleans. No secret
values or raw resource identifiers were printed or stored.

## Wake Context

- Issue: [LUC-1831](/LUC/issues/LUC-1831)
...
| Coolify resources list `GET /api/v1/resources` | pass; `17` visible rows, `8` matching production-environment rows |
