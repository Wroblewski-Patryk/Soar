# LUC-1641 Coolify Resource Inventory Reconciliation - 2026-06-03
...
Captured at `2026-06-03T02:34:56Z`.

| Probe | Result |
| --- | --- |
| `GET /api/issues/LUC-1641/heartbeat-context` | pass; issue `LUC-1641` read back as `blocked`, priority `critical`, zero first-class blockers |
| `GET /api/issues/LUC-1641/comments` | pass; only comment was a duplicate-owner live-run janitor note, not a Coolify access or inventory blocker |
...
| `GET /api/v1/resources` | pass; `17` visible rows; `9` Soar-relevant global rows in the allowlisted all-resource projection because Coolify exposes both `postgresql` and `postgresql-database-w5gql24ddjrgjaid7110rcqo` |
...
- PostgreSQL reports `running:healthy` with restart count `52` in global
  resource readback.
- Redis reports `running:healthy` with restart count `682` in global resource
  readback.
- `soar-api` reports restart count `5`; worker and web application rows report
  restart count `0` in the global resource projection.
