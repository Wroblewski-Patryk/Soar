# LUC-1997 Coolify Read-Only Production Status Access Evidence
...
At `2026-06-04T14:46:54Z`, authenticated read-only Coolify API calls confirmed
...
- Issue: [LUC-1997](/LUC/issues/LUC-1997)
...
- Latest comment impact: no comment delta in the wake payload; scoped wake
  ...
  lane. Heartbeat context later showed one recent issue comment but no
  first-class blocker.
...
| Check | Result |
| --- | --- |
| Paperclip heartbeat context `GET /api/issues/{issueId}/heartbeat-context` | pass; scoped issue context returned `in_progress`, priority `critical`, zero first-class blockers, and one recent comment |
