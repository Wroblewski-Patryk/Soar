# LUC-2181 Coolify Read-Only Production Status Access Evidence
...
At `2026-06-05T11:26:28Z`, authenticated read-only Coolify API calls confirmed
...
- Issue: [LUC-2181](/LUC/issues/LUC-2181)
...
- Present by name without value disclosure: `COOLIFY_BASE_URL`,
  `COOLIFY_API_TOKEN`, `COOLIFY_SOAR_PROJECT_ID`, and `COOLIFY_TOKEN`.
...
- Optional team binding absence is not an active blocker because
  project-scoped readbacks succeeded under selector `LuckySparrow`.
...
| Coolify teams read `GET /api/v1/teams` | pass; `1` team visible in this least-privilege runner |
...
| Coolify global resources list `GET /api/v1/resources` | pass; `1` visible row in this least-privilege runner; not used as release authority |
