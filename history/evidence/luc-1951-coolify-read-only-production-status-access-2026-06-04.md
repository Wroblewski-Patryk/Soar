# LUC-1951 Coolify Read-Only Production Status Access Evidence
...
At `2026-06-04T09:35:26Z`, authenticated read-only Coolify API calls confirmed
...
only binding names, counts, project/environment labels, resource names, and
resource status summaries. No secret values, raw resource identifiers,
generated database suffixes, cookies, tokens, or screenshots were stored.

## Wake Context

- Issue: [LUC-1951](/LUC/issues/LUC-1951)
...
| Coolify production environment read `GET /api/v1/projects/{configured-project-id}/production` | pass; six applications, PostgreSQL, and Redis |
| Coolify global resources list `GET /api/v1/resources` | pass; `17` visible rows, `6` Soar-relevant global rows by allowlisted name/FQDN heuristic |
