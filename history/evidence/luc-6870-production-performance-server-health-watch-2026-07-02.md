# LUC-6870 Production Performance And Server Health Watch
...
- Scoped issue: [LUC-6870](/LUC/issues/LUC-6870), status `in_progress`.
- Existing restoration blocker: [LUC-6331](/LUC/issues/LUC-6331), status `blocked`.
- Paperclip readback:
  - `/api/issues/{issueId}/heartbeat-context` -> `200`
  - `/api/issues/{issueId}` -> `200`
  - issue priority: `critical`
  - first-class blockers on [LUC-6870](/LUC/issues/LUC-6870): none before update
...
- Worktree: dirty before this heartbeat from existing lanes; this heartbeat
  added only scoped [LUC-6870](/LUC/issues/LUC-6870) evidence/task/state notes.
...
Command used existing smoke auth bindings already present in the runner
...
- worker heartbeat age: `13987 ms`
- market data age: `13987 ms`
...
- checked at: `2026-07-02T12:21:59.625Z`
- `shouldRollback`: `true`
...
- API `/health`: `200`, `0.593593s`
- API `/ready`: `200`, `0.163785s`
- Web `/`: `503`, `0.256964s`
- Web `/auth/login`: `503`, `0.496163s`
- Web `/api/build-info`: `503`, `0.358314s`
- API `/workers/ready` unauthenticated: `401`, `0.606469s`
...
- checked at: `2026-07-02T12:23:56.447Z`
- `/api/v1/version`: `200`
- `/api/v1/teams/current`: `200`, selector `0` / `LuckySparrow`
...
- data services:
  ...
  - `postgresql` companion row: `running:healthy`
...
resources remain unhealthy. This run did not reproduce a slow dashboard load;
the public Web failure is a fast `503`, so authenticated dashboard performance
cannot be accepted until Web service health is restored.
