# LUC-2465 Coolify Production Deploy Health Sweep

- Date: 2026-06-06
- Checked at: 2026-06-06T14:31:10Z
...
```json
{
  ...
  "checkedAt": "2026-06-06T14:31:10.776Z"
}
```

Note: a `curl.exe` spot-check pass returned `000` for API `/health` while the
project smoke script and a separate Node fetch both returned `200`; the Node
and project-script checks are the acceptance evidence for `/health`.
...
- Node public status probes -> API/Web public readiness PASS; unauthenticated worker readiness fail-closed with `401`.
