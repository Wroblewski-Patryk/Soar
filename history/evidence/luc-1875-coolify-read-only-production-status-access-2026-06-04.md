# LUC-1875 Coolify Read-Only Production Status Access Evidence
...
At `2026-06-04T02:35:44Z`, authenticated read-only Coolify API calls confirmed
...
- Issue: [LUC-1875](/LUC/issues/LUC-1875)
...
Note: the first production projection undercounted database/cache because it
read a generic `databases` field instead of Coolify's `postgresqls` and `redis`
buckets. The same read-only check was corrected immediately; no secret value was
printed and no production mutation occurred.
