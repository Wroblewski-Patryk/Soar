# LUC-2112 Coolify Read-Only Production Status Access Evidence
...
At `2026-06-05T07:23:02Z`, authenticated read-only Coolify API calls confirmed
...
resource classes, and application/data-service status summaries. No secret
values, raw resource identifiers, generated database suffixes, cookies, tokens,
or screenshots were stored.

## Wake Context

- Issue: [LUC-2112](/LUC/issues/LUC-2112)
...
| Data-service object-shape readback | pass; PostgreSQL reports `running:healthy`; Redis resource is present but status is not exposed by this redacted readback and is recorded as `running:unknown` |
...
| `redis` | standalone-redis | `running:unknown` | no |
...
project/environment/resource reconciliation. Application readiness, Redis
health beyond the inventory-presence signal, database health beyond the
inventory signal, protected worker readiness, SLO evidence, restore/rollback
evidence, and deploy mutation readiness remain separate release smoke
requirements and are not authorized by this issue.
