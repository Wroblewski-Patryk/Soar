# LUC-1497 Coolify Resource Inventory Reconciliation Evidence
...
- Issue: `LUC-1497` `[Ops][Soar] Reconcile Coolify resource inventory`
- Wake reason: `issue_continuation_needed` after a prior `process_lost_retry`
- Inline wake payload: `fallbackFetchNeeded=false`, pending comments `0/0`, latest comment id `unknown`
- The inline wake data contained no new comment batch. The fetched issue thread contained only duplicate-run janitor comments, so there was no product or operator comment to change the read-only inventory action.
- Checkout was already claimed by the harness and was not repeated.
- Paperclip heartbeat context showed the issue as `blocked` with no first-class blockers; this heartbeat treated that as process drift and completed the actionable read-only inventory proof.
...
| Check | Result |
| --- | --- |
| Paperclip heartbeat context `GET /api/issues/LUC-1497/heartbeat-context` | pass |
...
Fresh redacted readback timestamp: `2026-06-02T13:03:37Z`.
