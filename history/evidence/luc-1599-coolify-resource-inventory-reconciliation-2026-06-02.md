# LUC-1599 Coolify Resource Inventory Reconciliation - 2026-06-02
...
- `COOLIFY_SOAR_API_APP_ID`: present by name; value not printed.
- `COOLIFY_SOAR_WEB_APP_ID`: present by name; value not printed.
...
Captured at `2026-06-02T21:03:57Z`.

| Probe | Result |
| --- | --- |
| `GET /api/issues/{issue-id}/heartbeat-context` | pass; issue `LUC-1599` is `in_progress` with no first-class blockers and no comments |
| `GET /api/v1/teams/current` | pass; current selector id `0`, name `LuckySparrow` |
| `GET /api/v1/teams` | pass; visible teams include `LuckySparrow` and `empty - change it` |
...
| `GET /api/v1/resources` | pass; `17` visible resource rows; `9` Soar-relevant global rows because Coolify exposes both `postgresql` and `postgresql-database-w5gql24ddjrgjaid7110rcqo` |
...
resource. The extra global PostgreSQL row is recorded as a Coolify global-list
alias/companion row for reconciliation, not as an additional Soar deployable
