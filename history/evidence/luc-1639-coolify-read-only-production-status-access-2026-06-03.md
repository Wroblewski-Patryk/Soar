# LUC-1639 Coolify Read-Only Production Status Access Evidence
...
| Check | Result |
| --- | --- |
| Paperclip heartbeat context `GET /api/issues/LUC-1639/heartbeat-context` | pass; no comments and no first-class blockers |
| Names-only Coolify env binding check | pass: required names present without values printed |
| Coolify team list `GET /api/v1/teams` | pass; one visible team under this read-only token |
| Coolify current team `GET /api/v1/teams/current` | pass at `2026-06-03T02:34:15Z`; current selector id `0`, name `LuckySparrow` |
...
| Coolify resources list `GET /api/v1/resources` | pass; `1` visible global resource row under this read-only token |

Fresh redacted readback timestamp: `2026-06-03T02:34:15Z`.
