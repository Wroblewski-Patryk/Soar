...
| Type | Name | Status | Dockerfile | Public FQDN | Verification role |
| --- | --- | --- | --- | --- | --- |
| application | `soar-api` | `running:unknown` | `/apps/api/Dockerfile` | yes | API public/protected readiness |
| application | `soar-web` | `running:unknown` | `/apps/web/Dockerfile` | yes | Web route and build-info |
| application | `workers-backtest` | `running:unknown` | `/apps/api/Dockerfile.worker.backtest` | no | Backtest worker liveness |
| application | `workers-execution` | `running:unknown` | `/apps/api/Dockerfile.worker.execution` | no | Execution worker liveness |
| application | `workers-market-data` | `running:unknown` | `/apps/api/Dockerfile.worker.market-data` | no | Market-data worker liveness |
| application | `workers-market-stream` | `running:unknown` | `/apps/api/Dockerfile.worker.market-stream` | no | Market-stream worker liveness |
| standalone-postgresql | `postgresql` | `running:healthy` | n/a | no | Production database dependency |
| standalone-redis | `redis` | `running:healthy` | n/a | no | Cache/queue dependency |
...
plus PostgreSQL and Redis. There is no inventory drift from the prior LUC-1422
readback.
