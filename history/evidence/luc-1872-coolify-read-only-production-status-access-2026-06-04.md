# LUC-1872 Coolify Read-Only Production Status Access Evidence
...
At `2026-06-04T02:18:26Z`, authenticated read-only Coolify API calls confirmed
that this runner can read the configured Soar production status scope. The
proof used only binding names, counts, project/environment labels, resource
names, and resource status summaries. No secret values, raw resource
identifiers, generated database suffixes, cookies, tokens, or screenshots were
stored.

## Wake Context

- Issue: [LUC-1872](/LUC/issues/LUC-1872)
...
Note: the first PowerShell projection attempt failed before producing evidence
because a status expression was not compatible with the shell parser. The same
read-only checks were rerun successfully with a simpler helper; no secret value
was printed and no production mutation occurred.
...
Coolify's global resource endpoint also exposes a redacted PostgreSQL companion
row; this remains an alias/companion row, not a ninth production deploy or smoke
target.
